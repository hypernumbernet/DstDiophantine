...
/-! ### Nonzero commuting composite and the generated left ideal -/

/-- Working commuting projector \(P = P_R P_{\mathrm{spin,axis1}}\). -/
noncomputable def commutingSpinorIdem : PGA :=
  chiralityR * spinorIdemAxis1

theorem commutingSpinorIdem_sq :
    commutingSpinorIdem * commutingSpinorIdem = commutingSpinorIdem :=
  chiralityR_mul_spinorIdemAxis1_sq

private theorem one_add_chirality_mul_one_add_hyp1 :
    ((1 : PGA) + chiralityGen) * ((1 : PGA) + hyperbolic 1) =
      (1 : PGA) + chiralityGen + hyperbolic 1 + chiralityGen * hyperbolic 1 := by
  simp [mul_add, add_mul, one_mul, mul_one]
  abel

/-- A bivector cannot equal the scalar \(-1\). -/
private theorem iota0_mul_iota2_ne_neg_one : ι 0 * ι 2 ≠ (-1 : PGA) := by
  intro h
  have hrev : reverse (ι 0 * ι 2) = -(ι 0 * ι 2) :=
    reverse_ι_mul_ι 0 2 (by decide)
  rw [h] at hrev
  have hL : reverse (-1 : PGA) = -1 := by simp
  have hR : reverse (-1 : PGA) = 1 := by simpa using hrev
  have hPGA : (-1 : PGA) = 1 := hL.symm.trans hR
  have hR' : (-1 : ℝ) = 1 :=
    (FaithfulSMul.algebraMap_injective (R := ℝ) (A := PGA)
      (by simpa [map_neg, map_one] using hPGA))
  norm_num at hR'

private theorem commuting_num_ne_zero :
    ((1 : PGA) + chiralityGen) * ((1 : PGA) + hyperbolic 1) ≠ 0 := by
  intro hA
  have hsum :
      (1 : PGA) + chiralityGen + hyperbolic 1 + chiralityGen * hyperbolic 1 = 0 := by
    rw [← one_add_chirality_mul_one_add_hyp1, hA]
  have hα := congrArg (CliffordAlgebra.involute (Q := Q311)) hsum
  have hι1 : CliffordAlgebra.involute (Q := Q311) (PGA.ι 1) = -PGA.ι 1 :=
    CliffordAlgebra.involute_ι _
  have hι0 : CliffordAlgebra.involute (Q := Q311) (PGA.ι 0) = -PGA.ι 0 :=
    CliffordAlgebra.involute_ι _
  have hι2 : CliffordAlgebra.involute (Q := Q311) (PGA.ι 2) = -PGA.ι 2 :=
    CliffordAlgebra.involute_ι _
  have hch : CliffordAlgebra.involute (Q := Q311) chiralityGen = -chiralityGen := by
    simpa [chiralityGen] using hι1
  have hhyp : CliffordAlgebra.involute (Q := Q311) (hyperbolic 1) = hyperbolic 1 := by
    unfold hyperbolic
    rw [map_mul, hι0, hι2, neg_mul_neg]
  have hprod :
      CliffordAlgebra.involute (Q := Q311) (chiralityGen * hyperbolic 1) =
        -(chiralityGen * hyperbolic 1) := by
    rw [map_mul, hch, hhyp, neg_mul]
  have hαsum :
      (1 : PGA) - chiralityGen + hyperbolic 1 - chiralityGen * hyperbolic 1 = 0 := by
    have hexp : CliffordAlgebra.involute (Q := Q311)
        ((1 : PGA) + chiralityGen + hyperbolic 1 + chiralityGen * hyperbolic 1) =
          (1 : PGA) + (-chiralityGen) + hyperbolic 1 +
            (-(chiralityGen * hyperbolic 1)) := by
      simp [map_add, map_one, hch, hhyp, hprod]
    have h0 : CliffordAlgebra.involute (Q := Q311)
        ((1 : PGA) + chiralityGen + hyperbolic 1 + chiralityGen * hyperbolic 1) = 0 := by
      simpa [map_zero] using hα
    simpa [sub_eq_add_neg] using hexp.symm.trans h0
  have hsmul : (2 : ℝ) • ((1 : PGA) + hyperbolic 1) = 0 := by
    have h := congrArg₂ (· + ·) hsum hαsum
    rw [two_smul]
    convert h using 1 <;> abel
  have : hyperbolic 1 = (-1 : PGA) := by
    have hone : (1 : PGA) + hyperbolic 1 = 0 :=
      (smul_eq_zero.mp hsmul).resolve_left (by norm_num : (2 : ℝ) ≠ 0)
    exact eq_neg_of_add_eq_zero_right hone
  unfold hyperbolic at this
  exact iota0_mul_iota2_ne_neg_one this

theorem commutingSpinorIdem_ne_zero : commutingSpinorIdem ≠ 0 := by
  intro hP
  unfold commutingSpinorIdem chiralityR spinorIdemAxis1 at hP
  set A := ((1 : PGA) + chiralityGen) * ((1 : PGA) + hyperbolic 1)
  have hA : A * (half * half) = 0 := by
    simpa [A, mul_half_mul_half] using hP
  rw [half_mul_half] at hA
  have : algebraMap ℝ PGA (1 / 4) * A = 0 := by
    rw [algebraMap_comm]; exact hA
  have hsmul : (1 / 4 : ℝ) • A = 0 := by
    simpa [Algebra.smul_def] using this
  have hA0 : A = 0 :=
    (smul_eq_zero.mp hsmul).resolve_left (by norm_num : (1 / 4 : ℝ) ≠ 0)
  exact commuting_num_ne_zero (by simpa [A] using hA0)

/-- Principal left ideal generated by the commuting projector. -/
def leftIdealOf (e : PGA) : Set PGA :=
  { x | ∃ a, x = a * e }

theorem commutingSpinorIdem_mem_leftIdeal :
    commutingSpinorIdem ∈ leftIdealOf commutingSpinorIdem :=
  ⟨1, (one_mul _).symm⟩

/-- The left ideal generated by the commuting projector is nonzero.
Real dimension eight is proved in `Logic.Quantum.LeftIdealDim`.
Irreducibility of this ideal as a spinor module is not claimed. -/
theorem leftIdeal_commutingSpinorIdem_nontrivial :
...
Exact dimension eight is proved in `Logic.Quantum.LeftIdealDim`. -/
