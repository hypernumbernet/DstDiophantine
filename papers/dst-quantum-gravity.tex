...
For a four-momentum $p$ the grade-1 element $\gamma(p)=\sum_\mu p_\mu\gamma^\mu$ squares to the Minkowski quadratic $Q(p)=-p_0^2+p_1^2+p_2^2+p_3^2$, both in $\mathrm{Cl}(3,1)$ and as matrices on $\mathbb{C}^4$. The mass shell $Q(p)=-m^2$ is therefore the vanishing of $(\gamma(p)-im)(\gamma(p)+im)$. A spinor with $\gamma(p)\Psi=im\Psi$ lies on that shell: this is the Dirac equation in momentum space. In the Weyl representation $\gamma(p)$ is off-diagonal, so vanishing mass decouples the two chiral blocks while a nonzero mass mixes them. Chirality $\gamma^5$ sends a solution of mass $m$ to a solution of mass $-m$. Rest-frame solutions exist.
...
\]
Both angles accelerate together. The mismatch $\delta_a=\phi_a-\theta_a$ is therefore free: $\ddot\delta_a=0$. Affine lag $\delta_a(t)=\delta_{a0}+\nu_a t$ is allowed. The mass $m$ does not restore $\delta$ and does not fix a frequency~\cite{dst-dynamic-equation}.
...
An oscillator appears only if both kinetic signs are taken positive. The density $\tfrac12\dot\phi^2+\tfrac12\dot\theta^2-\tfrac m2(\phi-\theta)^2$ yields $\ddot\delta+2m\delta=0$, at frequency $\sqrt{2m}$ in units $\hbar=c=1$. That frequency is not the Compton frequency $m$, and the model is not the written action.

The written action therefore supplies no harmonic bridge to quantum theory. Linear accumulation of free mismatch is the only secular phase it actually yields. Identifying the rate $\nu$ with $E/\hbar$ remains a reading; the accumulated lag may then be written
\[
...
Dual-rotor matrix $U(\beta)\in\mathrm{SU}(2)$ at every dual rapidity; pure-dual Banach exponential matches & holds \\
...
Written opposite-sign action: $\ddot\phi=\ddot\theta=-m(\phi-\theta)$, hence $\ddot\delta=0$ & holds \\
Claimed EL $\ddot\phi=m(\phi-\theta)$, $-\ddot\theta=m(\phi-\theta)$ yields $\ddot\delta+m\delta=0$ & false (runaway $\ddot\delta=2m\delta$) \\
Same-sign kinetics: $\ddot\delta+2m\delta=0$ at $\sqrt{2m}$ & holds as a working model \\
