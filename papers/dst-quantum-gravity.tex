...
The product of $(1+e_1)/2$ with $(1+e_0 e_2)/2$ is idempotent; left ideal has real dimension $8$ & holds \\
...
Dual-rotor matrix $U(\beta)\in\mathrm{SU}(2)$ at every dual rapidity; relative dual rotors remain in $\mathrm{SU}(2)$ & holds \\
...
Momentum-space Dirac equation as Clifford factor of $Q(p)$ & holds \\
On-shell Weyl construction recovers rest $(u,iu)$ and solves every on-shell $p$ & holds \\
Rest-frame Dirac overlap is twice the dual-rotor amplitude & holds \\
Written opposite-sign action: $\ddot\delta=0$, $\ddot\sigma=-2m\delta$; Jacobi integral conserved and indefinite & holds \\
Claimed EL $\ddot\phi=m(\phi-\theta)$, $-\ddot\theta=m(\phi-\theta)$ yields $\ddot\delta+m\delta=0$ & false (runaway $\ddot\delta=2m\delta$) \\
Same-sign kinetics: $\ddot\delta+2m\delta=0$ at $\sqrt{2m}$, free common rapidity & holds as a working model \\
Spacetime Dirac PDE, mass as torsion, Higgs mechanism, measurement, path integral & open \\
Irreducibility of the left ideal as a spinor module & open \\
...
The double spacetime theory supplies an algebraic setting for quantum gravity, not a completed derivation. Every massive particle is encoded in the $16$-dimensional biquaternion algebra $\mathbb{H}\oplus\mathbb{H}\cong\mathrm{Cl}(3,1)$. The identities collected in Section~\ref{sec:status} include the admissible ceiling on $J$, the square-$+1$ projectors, on-axis commutativity only, the representation $-i\sigma$, the Rodrigues matrix of $\mathrm{SU}(2)$ at every dual rapidity, and the Clifford relations of $\mathrm{Cl}(3,1)$. Several natural identifications fail: the Killing pairing is not Born's rule, $I$ is not $i\sigma$, $\gamma^0$ is not $j$, $P_{\mathrm{spin}}P_R$ is not a projector, and $\mathrm{Cl}(3,1)$ is not $\mathrm{Cl}(9,1)$.

The spacetime Dirac PDE, fermion mass as torsional rigidity, the Higgs mechanism, measurement, and entanglement are not theorems of this paper. The momentum-space Dirac equation is the first-order factor of the Minkowski quadratic; on the mass shell it is solved by a dual spinor that recovers the rest pair $(u,iu)$. The written opposite-sign action frees the mismatch and sources the common rapidity, with a conserved indefinite Jacobi integral. A neighbouring oscillator exists only for same-sign kinetics, at frequency $\sqrt{2m}$, not at the Compton frequency $m$. Identifying free lag with the de Broglie phase remains a reading. Gravity as misalignment of usual and dual rotors remains the programme of the companion gravity papers; TEGR equivalence with Einstein--Hilbert is taken from the literature. The bound $|J|\le 3\pi^2/8$ constrains torsion on admissible configurations; it does not by itself forbid astrophysical singularities.
