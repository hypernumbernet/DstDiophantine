...
\]
Both angles accelerate together. The mismatch is therefore free: $\ddot\delta_a=0$. What the potential sources is the common rapidity, $\ddot\sigma_a=-2m\delta_a$. Affine lag $\delta_a(t)=\delta_{a0}+\nu_a t$ is allowed. A constant lag accelerates $\sigma$ uniformly; a nonzero rate makes that acceleration secular, so $\sigma$ grows as a cubic polynomial in $t$. The mass, written as a stiffness of $\delta$, is a force on $\sigma$ and does not restore a frequency~\cite{dst-dynamic-equation}.
...
An oscillator appears only if both kinetic signs are taken positive. The density $\tfrac12\dot\phi^2+\tfrac12\dot\theta^2-\tfrac m2(\phi-\theta)^2$ yields $\ddot\delta+2m\delta=0$, at frequency $\sqrt{2m}$ in units $\hbar=c=1$, while the common rapidity is free. That frequency is not the Compton frequency $m$, and the model is not the written action. For $m\ge 0$ its Jacobi integral is nonnegative, so conservation does bound the motion of $\delta$. The general solution is an oscillation of the mismatch together with uniform common rapidity.
...
The written opposite-sign particle action does not close the dynamical bridge from this kinematics to quantum theory. Its Euler--Lagrange system frees the mismatch and sources the common rapidity: $\ddot\delta=0$, $\ddot\sigma=-2m\delta$. The Jacobi integral is conserved and indefinite, so conservation does not restore a frequency. An oscillator appears only if both kinetic signs are taken positive, and then at $\sqrt{2m}$, not at the Compton frequency $m$. Identifying the rate of free lag with the de Broglie phase remains a reading. Mass as torsional rigidity, the Higgs mechanism, measurement as synchronization, entanglement, and the path integral are geometric hypotheses placed on this kinematics. They are not consequences of the algebra.
