...
In this paper we develop a radically different approach. We demonstrate that quantum gravity can be constructed entirely within the 16-real-dimensional biquaternion algebra isomorphic to the Clifford algebra $\mathrm{Cl}(3,1)$, without invoking a background manifold, Christoffel symbols, Riemann curvature, or any continuum structure. The sole physical realities are the intrinsic dual rotors carried by every massive particle: the usual-sector rotor $R_\text{usual}$ and its dual-sector counterpart $R_\text{dual}$. Quantum states emerge directly as rotations in the compact dual spacetime, while gravitational and inertial phenomena arise as torsional mismatch between the two rotors. The Lorentz-invariant scalar constructed from the Killing form on the relative Lie-algebra element $\Omega_\text{biv} = \log(R_\text{usual}^\dagger R_\text{dual})$ is the parameter-space form $J=\frac12\sum_a(\alpha_a^2-\beta_a^2)$. Its identification with the teleparallel scalar, and the ensuing Einstein--Hilbert equivalence, are taken from the TEGR literature after a rotor-to-tetrad dictionary is fixed.
...
\frac{1+g}{2}
\]
is idempotent. Working factors used in the formal development are the hyperbolic projector
\[
P_{\mathrm{spin}} = \frac{1+j}{2}
\]
(with $j^2=+1$) and a complementary spatial chirality pair
\[
P_L = \frac{1-e_1}{2},\qquad P_R = \frac{1+e_1}{2}
\]
(with $e_1^2=+1$), which satisfy $P_L^2=P_L$, $P_R^2=P_R$, and $P_L+P_R=1$. A composite primitive idempotent may then be taken in the product form
\[
P = P_{\mathrm{spin}}\, P_R
\]
when the factors commute (or otherwise associate) appropriately; the left ideal it generates is the spinor carrier
\[
...
\]
are \emph{not} idempotent: $\bigl((1-i)/2\bigr)^2=-i/2$. Composites such as $i\gamma_1\gamma_2$ that square to $+1$ remain admissible building blocks; the bare pseudoscalar does not.
...
\]
where $a_0,a_1,b_i,c_i \in \mathbb{R}$. Identifying the pseudoscalar $i$ with the imaginary unit of quantum mechanics, $\psi$ becomes a standard four-component complex column spinor in the Weyl basis. The first two components correspond to the usual-spacetime (left-handed) sector, while the remaining components encode the dual-spacetime (right-handed) sector. The dual map $X \mapsto X i$ still reverses time orientation and encodes chirality geometrically, but the algebraic projectors themselves are the square-$+1$ factors $P_L$, $P_R$ above—not $(1\pm i)/2$.
...
The dual map $X \mapsto X i$ reverses the time orientation; together with the square-$+1$ projectors $P_L=(1-e_1)/2$ and $P_R=(1+e_1)/2$ of Section~\ref{sec:spinor}, it organises the spinor space into chiral sectors. Consequently, the left-handed components couple preferentially to the usual-sector boosts while the right-handed components couple to the dual-sector rotations. This chiral asymmetry is not an additional assumption but a direct consequence of the time-reversed geometry of the dual spacetime, once the projectors are formed from generators with square $+1$.
...
Gravity itself is reinterpreted as the dynamical cost of misalignment between the usual and dual rotors. The Lorentz-invariant scalar $J$ constructed from the Killing form on the torsional mismatch bivector is identified with the teleparallel scalar only up to a total divergence after a rotor-to-tetrad dictionary is fixed; variational equivalence with the Einstein--Hilbert action is taken from the TEGR literature and is not re-derived here. Singularities are forbidden by the admissible ceiling $|J|\le 3\pi^2/8$, and event horizons do not form because light rays can tunnel through finite torsional layers.
