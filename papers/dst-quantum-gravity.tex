...
On the mass shell with $m\neq 0$ an arbitrary dual spinor $u$ determines a solution: the upper Weyl block is $u$, and the lower block is fixed by the chiral slash of $p$. At rest momentum $(m,0,0,0)$ this is the pair $\Psi=(u,iu)$. The same construction applies to every on-shell four-momentum, including the longitudinal boost
\[
p=(m\cosh\alpha,\,0,\,0,\,m\sinh\alpha).
\]
The $\mathbb{C}^4$ overlap of two rest-frame solutions that differ by a dual rotation is twice the dual-rotor amplitude. That overlap is a rest-frame identity; it is not a spacetime inner product.

A spacetime PDE, the mass term as dual-rotor lag, and covariance under the full rotor group are not derived here. Square-$+1$ projectors $P_L=(1-e_1)/2$ and $P_R=(1+e_1)/2$ organise a chiral split in the algebra; they do not by themselves yield the Standard Model weak interaction.
