...
On admissible configurations the torsional scalar obeys $|J|\le 3\pi^2/8$, or equivalently $|J_{\mathrm{norm}}|\le 1$. Square-$+1$ generators yield idempotent projectors $P_L$, $P_R$ and $P_{\mathrm{spin}}$; the naive factors $(1\pm i)/2$ with $i^2=-1$ do not, nor does the composite $P_{\mathrm{spin}}P_R$. Usual and dual generators commute on a common axis and need not commute off-axis. The cyclic generators satisfy $IJ=K$ and act on $\mathbb{C}^2$ by $-i\sigma_a$; at every dual rapidity the dual rotor is the Rodrigues matrix of $\mathrm{SU}(2)$, which is unitary of determinant one. The grade-1 generators of $\mathrm{Cl}(3,1)$ obey $\{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}$ with $\eta=\mathrm{diag}(-1,+1,+1,+1)$, and $\gamma^0$ is not the hyperbolic generator $j$ used for $P_{\mathrm{spin}}$. The Killing self-pairing is indefinite and is not a Born probability~\cite{dst-d4l}. The algebras $\mathrm{Cl}(3,1)$ and $\mathrm{Cl}(9,1)$ are not isomorphic, and the dual spinor space is not the Majorana--Weyl module of ten-dimensional spacetime. Variational equivalence of TEGR with the Einstein--Hilbert action is taken from the literature after a rotor-to-tetrad dictionary is fixed. The momentum-space Dirac equation is the first-order factor of the Minkowski quadratic in \(\mathrm{Cl}(3,1)\); on the mass shell a dual spinor recovers the rest-frame pair \((u,iu)\), and a longitudinal boost of that pair is the same on-shell construction after a chiral factor along the boost axis. The written opposite-sign action frees the mismatch and sources the common rapidity. Affine lag integrates to a cubic common rapidity, and the Jacobi integral along that motion reduces to initial data. The integral is indefinite, so conservation does not restore a frequency. The harmonic stiffness does not select a misalignment. Measurement and the path integral remain geometric hypotheses.
\end{abstract}
...
At every dual rapidity the dual rotor is the Rodrigues matrix of $\mathrm{SU}(2)$: unitary of determinant $1$, with relative dual rotors remaining in the group. This is the matrix representation on $\mathbb{C}^2$ recorded in Section~\ref{sec:quantumstates}, not a Riemannian identification of the rotor manifold with the three-sphere. The Dirac matrices of signature $(-,+,+,+)$ are the grade-1 generators of $\mathrm{Cl}(3,1)$; they obey $\{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}$ and are not the hyperbolic bivectors used for $P_{\mathrm{spin}}$. Born's rule is not the Killing self-pairing: that pairing has signature $(3,3)$ and is negative on pure elliptic configurations~\cite{dst-d4l}. Position and momentum operators, a spacetime Dirac PDE, and mass as torsion are not derived here. The momentum-space Dirac equation follows from the Clifford square of the Minkowski quadratic, as recorded in Section~\ref{sec:dirac}.

By abandoning the continuum hypothesis entirely, the theory automatically resolves the singularities of classical general relativity. The compactness of the dual spacetime discretizes the rotor angles. On admissible configurations the algebraic ceiling is $|J|\le 3\pi^2/8$ (equivalently $|J_{\rm norm}|\le 1$); this forbids the unbounded curvature that would otherwise appear at $r=0$ or at the Big Bang, provided the configuration remains admissible. Event horizons are likewise absent: light rays traverse finite torsional layers via resonance tunneling. Strong-field deviations from general relativity become observable in principle. Dual-only kinematics on the admissible cone delimit when $J$ can be shielded or reversed, and that the dual budget may be routed across silent axes; whether laboratory fields supply that dual force is the experimental question of Section~\ref{sec:experiments}.

The remainder of the paper is organized as follows. Section~\ref{sec:spinor} constructs square-$+1$ projectors and shows that the composite $P_{\mathrm{spin}}P_R$ is not idempotent. Section~\ref{sec:quantumstates} identifies dual generators with $-i\sigma_a$ and the dual rotor at every rapidity with an $\mathrm{SU}(2)$ matrix, and shows that the Killing pairing is not Born's rule. Section~\ref{sec:dirac} records the Clifford relations of $\mathrm{Cl}(3,1)$, distinguishes $\gamma^0$ from the hyperbolic generator $j$, factors the Minkowski quadratic into the momentum-space Dirac equation, assembles on-shell solutions from a dual spinor, and identifies a longitudinal boost of the rest pair with that construction. Sections~\ref{sec:mass}--\ref{sec:dynamics} record that the harmonic stiffness does not select a misalignment, and discuss measurement and the path integral as geometric hypotheses; the written particle action is shown to free the mismatch and source the common rapidity. Section~\ref{sec:predictions} presents conjectural predictions bounded by the admissible ceiling. Section~\ref{sec:comparison} compares the framework with other programmes, distinguishing internal duality from ten-dimensional spacetime supersymmetry. Section~\ref{sec:experiments} outlines near-term tests. Section~\ref{sec:status} gathers the algebraic picture of the quantum sector. Section~\ref{sec:conclusions} summarizes.
...
\]
is not idempotent: $e_0 e_1$ and $e_1$ anticommute, the factors do not commute, and $(P_{\mathrm{spin}}P_R)^2\neq P_{\mathrm{spin}}P_R$. A commuting square-$+1$ pair is $e_1$ with the off-axis boost $e_0 e_2$; their product projector is idempotent and generates a left ideal of real dimension eight. That ideal is the direct sum of the four-dimensional Minkowski core $\mathrm{Cl}(3,1)\,P$ and a copy along the null generator $e_4$; the copy is a proper $G(3,1,1)$-submodule, so the eight-dimensional ideal is reducible as a PGA-module. The match with the real dimension of a Dirac spinor $\mathbb{C}^4$ is this null doubling, not an identification with an irreducible Dirac module. Irreducibility of the Minkowski core as a $\mathrm{Cl}(3,1)$-module is not claimed.
...
At every dual rapidity $\beta$ the exponential is the Rodrigues formula
\[
U(\beta)
=\cos(\lVert\beta\rVert/2)\,I
+\sin(\lVert\beta\rVert/2)\,n\cdot(-i\sigma),
\]
with $n=\beta/\lVert\beta\rVert$ when $\beta\neq 0$ and $U(0)=I$. That matrix is unitary of determinant $1$. Relative dual rotors remain in $\mathrm{SU}(2)$. The inverse is the opposite rapidity, so a relative dual rotor is a product in the group: its character records the alignment of the two dual axes, and the noncommuting residue is a traceless tilt. A $2\pi$ dual rotation about a computational axis is $-I$. Compactness of the dual spacetime ($\phi_a \bmod 4\pi$) is compatible with this matrix group on $\mathbb{C}^2$; an independent diffeomorphism of manifolds $S^3\cong\mathrm{SU}(2)$ is not proved here.
...
Because the dual rotor is the $\mathrm{SU}(2)$ Rodrigues matrix at every rapidity, the dual sector can host spin-$1/2$ kinematics; a full identification of Standard Model fermions with dual phases is not claimed. The dual map $X\mapsto Xi$ is an internal automorphism of the algebra. It is not spacetime supersymmetry of a ten-dimensional string: $\mathrm{Cl}(3,1)\not\cong\mathrm{Cl}(9,1)$, the dual spinor space is not the Majorana--Weyl module of $\mathrm{Cl}(9,1)$, and the Super-Poincar\'e generator count is not the ten generators of the projective geometric algebra.

The opposing signatures cancel when the sectors align. That cancellation does not generate the observed fermion masses, and the admissible bound on \(J\) is not the electroweak scale. A harmonic stiffness of the mismatch does not select a vacuum away from alignment.
...
For a four-momentum $p$ the grade-1 element $\gamma(p)=\sum_\mu p_\mu\gamma^\mu$ squares to the Minkowski quadratic $Q(p)=-p_0^2+p_1^2+p_2^2+p_3^2$, both in $\mathrm{Cl}(3,1)$ and as matrices on $\mathbb{C}^4$. The two chiral slashes of the Weyl representation multiply to that same quadratic. The mass shell $Q(p)=-m^2$ is therefore the vanishing of $(\gamma(p)-im)(\gamma(p)+im)$. A spinor with $\gamma(p)\Psi=im\Psi$ lies on that shell: this is the Dirac equation in momentum space. In the Weyl representation $\gamma(p)$ is off-diagonal, so vanishing mass decouples the two chiral blocks while a nonzero mass mixes them. Chirality $\gamma^5$ sends a solution of mass $m$ to a solution of mass $-m$.

On the mass shell with $m\neq 0$ an arbitrary dual spinor $u$ determines a solution: the upper Weyl block is $u$, and the lower block is fixed by the chiral slash of $p$. At rest momentum $(m,0,0,0)$ this is the pair $\Psi=(u,iu)$. Dual rotation of $u$ remains a solution at the same four-momentum: the dual rotor acts internally on the Weyl parameter.

A longitudinal boost of rest momentum,
\[
p=(m\cosh\alpha,\,0,\,0,\,m\sinh\alpha),
\]
stays on the shell. At that momentum the Weyl construction is the Dirac-boosted rest-frame pair. The boost acts by the hyperbolic Rodrigues factors
\[
V(\alpha)=\cosh(\alpha/2)\,I+\sinh(\alpha/2)\,\sigma_z
\]
on the upper Weyl block and $V(-\alpha)$ on the lower: the rest pair $(u,iu)$ is sent to the on-shell solution whose Weyl parameter is $V(\alpha)u$. Dual rotation about the boost axis commutes with $V(\alpha)$, so internal dual rotation and the longitudinal boost may be taken in either order. Dual rotation about a perpendicular axis does not commute with $V(\alpha)$. This is not covariance under the full rotor group: mixed usual/dual factorisation and a spacetime PDE remain underived.

The $\mathbb{C}^4$ overlap of two rest-frame solutions that differ by a dual rotation is twice the dual-rotor amplitude. That overlap is a rest-frame identity; it is not a spacetime inner product, and it is not claimed after the boost.

Square-$+1$ projectors $P_L=(1-e_1)/2$ and $P_R=(1+e_1)/2$ organise a chiral split in the algebra; they do not by themselves yield the Standard Model weak interaction.

\section{Torsional Stiffness and the Higgs Reading}
\label{sec:mass}

Section~\ref{sec:dirac} gives the momentum-space Dirac equation as a Clifford factor of $Q(p)$. What follows is a geometric hypothesis for the \emph{value} of $m$, not a derivation of that value from torsion.

The torsional mismatch between the usual-sector rotor $R_\text{usual}$ and the dual-sector rotor $R_\text{dual}$ is quantified by the relative angle $\delta_a = \phi_a - \theta_a$. A modelling stiffness proportional to the rest mass $m$ may be assigned to that angle. In the low-energy limit the associated potential is
\[
V_\text{torsion} = \frac{1}{2} m \sum_{a=1}^3 \delta_a^2.
\]
This $V_{\mathrm{torsion}}$ is a particle-intrinsic oscillator potential, not the parameter-space Killing quadratic of the main paper, and it is not a restoring force in the Euler--Lagrange system of the written opposite-sign action of Section~\ref{sec:dynamics}. The bilinear
\[
m \bar{\psi}\psi \propto \operatorname{Tr}(\Omega_\text{biv}),
\]
with $\Omega_\text{biv} = \log(R_\text{usual}^\dagger R_\text{dual})$, is a geometric reading of the Dirac mass, not a derivation of $m$ from the lag.

The harmonic stiffness is not that mechanism. In the standard model the Higgs field acquires a vacuum expectation value away from the symmetric point, and the curvature there generates masses. Here the written potential is minimized at alignment. Under the opposite kinetic signs of the particle action every constant lag is free, and the lag is inertial only where the slope vanishes, so a nonzero harmonic lag accelerates the common rapidity rather than settling into a misalignment. A quartic well off the origin can freeze a nonzero lag, but only when the two kinetic signs agree; the common phase is then flat, and its rate is initial data, not a mass for \(W\) or \(Z\). No Weinberg angle is produced. The compact dual sector still bounds the kinematic mismatch on the admissible cone. That bound is not a derivation of the electroweak scale from the Planck scale, and it is not a substitute for the missing vacuum.

\section{Unified Dynamics of Quantum Mechanics and Gravity}
\label{sec:dynamics}

The rotor Lagrangian is a model for the six rapidities, not a deduction from unitarity of the rotors. Schr\"odinger evolution, measurement as synchronization, entanglement, and the path integral below remain geometric hypotheses. In that picture, dual-rotor kinematics is the quantum side and usual--dual misalignment is the gravitational side.

Consider a single massive particle whose complete rotor is $R_\text{total} = R_\text{usual} R_\text{dual}$. The written effective action for the intrinsic rotor angles is
\[
...
\]
The opposite signs of the kinetic terms are part of the written model. They are intended to reflect the boost-like (hyperbolic) character of the usual sector and the rotation-like (elliptic) character of the dual sector; they are not deduced from unitarity of the rotors. The last term is the oscillator potential of Section~\ref{sec:mass}, not the parameter-space Killing quadratic of the main paper.

On one axis the density is equivalently the mixed form
\[
L=\tfrac12\dot\sigma\dot\delta-\tfrac m2\delta^2,
\]
with common rapidity $\sigma=\phi+\theta$ and mismatch $\delta=\phi-\theta$. The Euler--Lagrange equations of this density are
\[
\ddot{\phi}_a = -m (\phi_a - \theta_a), \qquad \ddot{\theta}_a = -m (\phi_a - \theta_a).
\]
Both angles accelerate together. The mismatch is therefore free: $\ddot\delta_a=0$. What the potential sources is the common rapidity, $\ddot\sigma_a=-2m\delta_a$. The free mismatch integrates at once. On one axis the general solution is the affine lag $\delta(t)=\delta_0+\nu t$, and the common rapidity is the cubic
\[
\sigma(t)=\sigma_0+\dot\sigma_0\,t-m\delta_0\,t^2-\frac{m\nu}{3}\,t^3.
\]
A frozen lag accelerates $\sigma$ uniformly; a nonzero rate makes the acceleration itself linear in time. Along this motion the Jacobi integral collapses to the initial value $\tfrac12\nu\dot\sigma_0+\tfrac m2\delta_0^2$: the quadratic growth of $\dot\sigma$ cancels the quadratic growth of the potential, so the charge stays finite while the rapidity runs away. The mass, written as a stiffness of $\delta$, is a force on $\sigma$ and does not restore a frequency~\cite{dst-dynamic-equation}.
...
\]
is conserved along those equations and is indefinite: a purely dual velocity at vanishing lag already yields $E<0$. Conservation of an unbounded quadratic does not select a frequency.

A neighbouring system sometimes written alongside the action,
\[
\ddot{\phi}_a = m (\phi_a - \theta_a), \qquad -\ddot{\theta}_a = m (\phi_a - \theta_a),
\]
makes the mismatch runaway, $\ddot\delta_a=2m\delta_a$. It is not an oscillator.

An oscillator appears only if both kinetic signs are taken positive. The density $\tfrac12\dot\phi^2+\tfrac12\dot\theta^2-\tfrac m2(\phi-\theta)^2$ yields $\ddot\delta+2m\delta=0$, at frequency $\sqrt{2m}$ in units $\hbar=c=1$, while the common rapidity is free. The general solution is $\delta(t)=A\cos(\sqrt{2m}\,t)+B\sin(\sqrt{2m}\,t)$, with $\delta(t)^2\le A^2+B^2$, and its Jacobi integral equals $\tfrac14\dot\sigma_0^2+\tfrac m2(A^2+B^2)$. That frequency and the Compton frequency $m$ meet only at the isolated values $m=0$ and $m=2$, and the model is not the written action. For $m\ge 0$ its Jacobi integral is nonnegative, so conservation does bound the motion of $\delta$. The neighbouring sign pattern $\ddot\delta=2m\delta$, at the same rate, is instead the unbounded hyperbolic motion $A\cosh(\sqrt{2m}\,t)+B\sinh(\sqrt{2m}\,t)$.

The written action therefore supplies no harmonic bridge to quantum theory. Identifying the rate $\nu$ of free lag with $E/\hbar$ remains a reading; the accumulated lag may then be written
\[
\int \delta_a \, dt \propto \frac{E t - \mathbf{p} \cdot \mathbf{x}}{\hbar},
\]
the de Broglie phase of the matter wave~\cite{dst-debroglie}. The scalar $\mathrm{U}(1)$ factor built from $\delta$ is distinct from the real character of the dual rotor and from its spinor matrix element.

The Schr\"odinger equation is readable as the unitary evolution of the dual rotor under an external potential that modulates the dual phases. Compactness of the dual sector would keep that evolution unitary. It is not derived from the written particle action.

Wave-function collapse is readable as forced synchronization of dual rotors across an entangled many-particle system. A measurement interaction that coupled to the usual spacetime would still have to supply a restoring mechanism: the written action does not drive $\delta_a$ toward zero, and minimizing a global $J$ is a further identification, not an Euler--Lagrange consequence. The apparent non-unitary jump would then be the classical limit of a rapid, collective torsional relaxation. No additional collapse postulate is required in that picture; the picture itself remains a hypothesis.

Quantum entanglement likewise acquires a torsional reading. When two particles share correlated dual phases, their collective mismatch energy is lowered only after a potential for that mismatch is chosen. A local operation on one particle that alters its dual rotor would then change a global $J$, producing the instantaneous correlation characteristic of entanglement without violating causality (information propagates at most at the speed of light through the usual sector). Compactness of the dual sector makes that non-locality available as a geometric option, not as a theorem of the written action.

A path-integral formulation may be written with the same particle action,
\[
\langle R_f | R_i \rangle = \int \mathcal{D}[\phi,\theta] \, \exp\left( \frac{i}{\hbar} S_\text{particle}[\phi,\theta] \right),
\]
the integral running over paths in the dual-rotor phase space. Compactness of the dual sector would keep the integral well-defined. Because the written $S_{\mathrm{particle}}$ makes $\delta$ free, the torsional potential term does not by itself generate Compton-frequency interference.

In this picture quantum mechanics and gravity are not separate theories that must be reconciled. Quantum mechanics is the kinematics of rotations in the compact dual spacetime; gravity is the dynamical cost of breaking the parallelism between the usual and dual rotors. Both readings originate from the same 16-dimensional biquaternion algebra. The kinematics is sharp. The dynamical bridge from the written particle action is not.
...
The double spacetime theory recovers the Einstein equations precisely through its equivalence with teleparallel gravity, while a commuting projector in $G(3,1,1)$ generates an eight-dimensional left ideal that splits by the null generator and is not read as an irreducible fermion. Discreteness is algebraic (Diophantine constraints on rotor angles) rather than set-theoretic, and causality is preserved by the lightlike $J=0$ resonance condition. No thermodynamic analogy is invoked; the emergence of spacetime is a collective effect of torsional alignment among particle-intrinsic dual rotors.
...
Although the double spacetime theory is fundamentally algebraic, its quantum sector makes concrete experimental predictions that can be tested with current or near-future technology. The mismatch scalar $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ depends only on the relative rotor. Dual-only motion cannot raise $J$; it conserves $J+M$, so every dual-only shield from a pure usual seed doubles unsigned mass. Shielding is a global budget, $\sum\alpha\le 3\pi/4$, not an isotropic per-axis condition: a single boosted axis can still be dual-only shielded by parking dual rapidity on a silent axis. Repulsion requires a strictly negative wall. The vacuum $J=M=0$ is mixed-reachable and dual-only unreachable except from the origin. The dual-only increment of the dual angles is helicity-even in $J$: a circular Faraday wave does not displace the mean, while a circular potential drops it ponderomotively and leaves the cone from a vanishing dual seed. Whether laboratory fields supply a helicity-odd dual-channel force remains a conjecture.

\subsection{Theoretical Coupling Mechanism}

A circularly polarised electromagnetic wave is null at every phase, with vanishing four-phase mean of every linear Faraday coefficient. Superposing that wave onto the mismatch does not shift mean $J$. Among the six classical Faraday quadratics, the unique helicity-odd combination on that wave is the Poynting component $(E\times B)_z=\sigma E_0^2$. The same dual-only increment, read as $\theta\mapsto\theta+eA$, drops mean $J$ by the helicity-even ponderomotive $\tfrac12 e^2\langle A^2\rangle$ and is not Maxwell-gauge invariant. Helicity drive of the particle mismatch is neither that increment nor a local scalar of $F$. The working model takes the Poynting bilinear as the candidate source of a dual-phase drive, with right-handed polarisation ($\sigma=+1$) reinforcing attractive mismatch ($J>0$) and left-handed polarisation ($\sigma=-1$) opposing it. The effective interaction is written
\[
H_\text{int} = \kappa \sigma E_0^2 \sum_a \theta_a \Gamma_a,
\]
where $E_0$ is the electric-field amplitude and $\kappa$ is a material-dependent coupling, neither of which is derived from the dual-rotor algebra. The phase-lock
\[
\dot{\theta} = \kappa \sigma E_0^2 \sin(\omega t - \theta + \delta_\sigma)
\]
is extra structure. The written particle action, given a dual-channel acceleration $u$, makes the mismatch a double integrator $\ddot\delta=-u$ and supplies no restoring frequency. Dual-only kinematics is a constrained slice of that action; without the constraint, both sectors move.
...
\item Look for helicity-dependent changes in effective mass. A resonant peak at a proposed dual-rotor scale $\theta \sim c / (2\pi r_\text{comp})$ would test the extra phase-lock structure, not the dual-only geometry of $J$, and not the helicity-even ponderomotive of the dual-only increment.
\end{itemize}

Predicted effect sizes, if the coupling exists, are $\Delta m / m \sim 10^{-2}$--$10^{-5}$ at achievable intensities of $10^{12}$--$10^{15}$ W/m$^2$. The signature is distinctive: the effect must reverse sign when the polarization helicity is flipped and must vanish for linearly polarized or unpolarized radiation.

\subsection{Falsifiability and Caveats}

A null result at the $10^{-6}$ level after a systematic frequency and helicity scan would constrain the dual-rotor coupling strength, not the admissible-cone kinematics of $J$. Positive detection---particularly a helicity-reversible mass shift---would constitute strong evidence that gravity and inertia are torsional phenomena originating in dual spacetime. A resonance peak would further constrain the working oscillator or phase-lock, which the written action does not force.

The theory makes no claim of immediate technological breakthrough. The proposals above are modest, table-top experiments whose primary purpose is to test the hypothesis that dual-spacetime degrees of freedom couple measurably to laboratory electromagnetic fields. Success at this level would open the door to larger-scale gravitational engineering; failure would simply bound the parameters of the coupling model.
...
In summary, dual-only kinematics already delimit when $J$ can be shielded or reversed, at what unsigned-mass cost, and that the dual budget may be routed across silent axes. Whether nature supplies a laboratory dual force---and whether gravity is therefore a controllable torsional degree of freedom---is the experimental question of the coming decade.

\section{The algebraic picture}
\label{sec:status}

The quantum sector of double spacetime theory is the kinematics of a compact dual spacetime already present in the $16$-real-dimensional algebra $\mathbb{H}\oplus\mathbb{H}\cong\mathrm{Cl}(3,1)$. Gravity is the dynamical cost of breaking the parallelism between the usual and dual rotors that every massive particle carries. No background manifold is quantized. The Hilbert-layer pairing is treated in~\cite{dst-d4l}.

The dual sector is elliptic. Its generators square to $-1$, satisfy $IJ=K$, and act on $\mathbb{C}^2$ by $-i\sigma_a$. At every dual rapidity the dual rotor is an $\mathrm{SU}(2)$ matrix, unitary of determinant one; relative dual rotors remain in the group. The inverse is the opposite rapidity, so a relative dual rotor is a product whose character records the alignment of the two dual axes, with a traceless noncommutative remainder. Compactness of the dual spacetime is compatible with this matrix representation. This is the spin-$1/2$ kinematics of the theory: a representation on $\mathbb{C}^2$, not a Riemannian identification of the rotor manifold with the three-sphere, and not a derivation of position and momentum operators. Usual and dual generators commute on a common axis and need not commute off-axis, so an unrestricted factorization $R_{\mathrm{total}}=R_{\mathrm{usual}}R_{\mathrm{dual}}$ is not an identity for arbitrary axes.

The Dirac matrices of signature $(-,+,+,+)$ are the grade-1 generators of the same algebra. They are not the hyperbolic bivectors used to build square-$+1$ projectors: $\gamma^0$ squares to $-1$, while the generator $j$ of $P_{\mathrm{spin}}$ squares to $+1$. Fermionic kinematics therefore sits inside the algebra. A commuting square-$+1$ projector generates an eight-dimensional left ideal in $G(3,1,1)$ that splits as a four-dimensional Minkowski core plus a null copy; the copy is a proper submodule, so the ideal is reducible as a PGA-module. The real dimension matches $\mathbb{C}^4$ by that doubling, not by identification with an irreducible Dirac module. A dual spinor is already a $\mathbb{C}^2$ vector for the cyclic generators, and the Dirac carrier is assembled from it on the mass shell. The Clifford square of a four-momentum is the Minkowski quadratic $Q(p)$. The momentum-space Dirac equation is the first-order factor of that quadratic. On the mass shell a dual spinor $u$ determines a solution; at rest momentum this is the pair $\Psi=(u,iu)$. Dual rotation of $u$ stays on that shell. A longitudinal boost of the rest pair is the same Weyl construction after a chiral factor along the boost axis, which commutes with dual rotation about that axis and need not commute with a perpendicular dual rotation. The rest-frame overlap of two such solutions that differ by a dual rotation is twice the dual-rotor amplitude. None of this is a spacetime PDE, none of it is covariance under the full rotor group, and none of it derives the value of $m$ from torsion.

Gravity is the mismatch of the two rotors, measured by the parameter-space scalar $J=\frac12\sum_a(\alpha_a^2-\beta_a^2)$. On admissible configurations $|J|\le 3\pi^2/8$, or equivalently $|J_{\mathrm{norm}}|\le 1$. Compactness does not force an algebraic ceiling of order $N^{-2}$: the discrete quantity $\varepsilon_N=16/(3N^2)$ is a lower bound on nonzero normalized height. Vacuum suppression, if it occurs, must come from a dynamical cancellation between the hyperbolic and cyclic sectors, whose Killing form takes opposite signs. The Killing self-pairing itself is indefinite of signature $(3,3)$ and is not a Born probability~\cite{dst-d4l}. Identification of $J$ with the teleparallel scalar, and variational equivalence with the Einstein--Hilbert action, are taken from the literature after a rotor-to-tetrad dictionary is fixed~\cite{maluf2013}. The dual map $X\mapsto Xi$ is an internal automorphism of $\mathrm{Cl}(3,1)$. It is not spacetime supersymmetry of a ten-dimensional string: $\mathrm{Cl}(3,1)\not\cong\mathrm{Cl}(9,1)$, and the dual spinor space is not the Majorana--Weyl module of $\mathrm{Cl}(9,1)$.

The written opposite-sign particle action does not close the dynamical bridge from this kinematics to quantum theory. Its Euler--Lagrange system frees the mismatch and sources the common rapidity: $\ddot\delta=0$, $\ddot\sigma=-2m\delta$. The integral is an affine lag and a cubic common rapidity, and the Jacobi integral along that motion reduces to initial data. The integral is indefinite, so conservation does not restore a frequency. An oscillator appears only if both kinetic signs are taken positive, and then at $\sqrt{2m}$, which meets the Compton frequency $m$ only at the isolated values $m=0$ and $m=2$. Identifying the rate of free lag with the de Broglie phase remains a reading. The harmonic stiffness does not select a misalignment: a nonzero lag is inertial only where the slope of the potential vanishes. Measurement as synchronization, entanglement, and the path integral remain geometric hypotheses placed on this kinematics. They are not consequences of the algebra.

The continuum remains a classical approximation. The $16$-dimensional algebra is the setting of the quantum sector, not a finished theory of quantum gravity.

\section{Conclusions}
\label{sec:conclusions}

The double spacetime theory supplies an algebraic setting for quantum gravity, not a completed derivation. Quantum kinematics is the compact dual sector of $\mathrm{Cl}(3,1)$; gravity is the mismatch of usual and dual rotors. Section~\ref{sec:status} records what that algebra determines: the $\mathrm{SU}(2)$ dual rotor, the momentum-space Dirac equation solved on the mass shell by a dual spinor that recovers the rest pair $(u,iu)$, the longitudinal boost of that pair, and the admissible ceiling on $J$. It does not determine a spacetime Dirac PDE, covariance under the full rotor group, a Compton oscillator from the written action, or mass as torsion.

Predictions and laboratory proposals in the preceding sections remain conjectural, bounded by that ceiling. Dual-sector kinematics is the $\mathbb{C}^2$ representation described in~\cite{dst-d4l}. The continuum remains a classical approximation. The $16$-dimensional algebra is the setting, not a finished theory of quantum gravity.
