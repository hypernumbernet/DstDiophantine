...
\]
with $n=\beta/\lVert\beta\rVert$ when $\beta\neq 0$ and $U(0)=I$. That matrix is unitary of determinant $1$. Relative dual rotors remain in $\mathrm{SU}(2)$. The inverse is the opposite rapidity, so a relative dual rotor is the product $R(-\beta)R(\beta')$. Along a common ray that product is a single dual rotation by the difference of the scales. Off-axis its half-trace records the alignment of the two dual axes, while the noncommuting residue is a traceless tilt of the composite axis; a pair of $\pi$ dual rotations about perpendicular axes has group commutator $-I$. A $2\pi$ dual rotation about a computational axis is $-I$. Compactness of the dual spacetime ($\phi_a \bmod 4\pi$) is compatible with this matrix group on $\mathbb{C}^2$; an independent diffeomorphism of manifolds $S^3\cong\mathrm{SU}(2)$ is not proved here.
