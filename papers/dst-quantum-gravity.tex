...
Although the double spacetime theory is fundamentally algebraic, its quantum sector makes concrete experimental predictions that can be tested with current or near-future technology. The mismatch scalar $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ depends only on the relative rotor. Dual-only motion cannot raise $J$; it conserves $J+M$, so every dual-only shield from a pure usual seed doubles unsigned mass. Shielding is a global budget, $\sum\alpha\le 3\pi/4$, not an isotropic per-axis condition: a single boosted axis can still be dual-only shielded by parking dual rapidity on a silent axis. Repulsion requires a strictly negative wall. The vacuum $J=M=0$ is mixed-reachable and dual-only unreachable except from the origin. Whether laboratory fields supply a dual-channel force remains a conjecture.

\subsection{Theoretical Coupling Mechanism}

A circularly polarised electromagnetic wave is null at every phase, with vanishing four-phase mean of every linear Faraday coefficient. Among the six classical Faraday quadratics, the unique helicity-odd combination on that wave is the Poynting component $(E\times B)_z=\sigma E_0^2$. Helicity drive of the particle mismatch is not a local scalar of $F$, nor a linear superposition, nor a rest sandwich of the dual summand. The working model takes that bilinear as the candidate source of a dual-phase drive, with right-handed polarisation ($\sigma=+1$) reinforcing attractive mismatch ($J>0$) and left-handed polarisation ($\sigma=-1$) opposing it. The effective interaction is written
\[
...
\]
is extra structure. The written particle action, given a dual-channel acceleration $u$, makes the mismatch a double integrator $\ddot\delta=-u$ and supplies no restoring frequency. Holding $\ddot\phi=0$ uniquely fixes a usual-channel constraint force $v=m(\phi-\theta)$; without that constraint, both sectors move.
...
\item Look for helicity-dependent changes in effective mass. A resonant peak at a proposed dual-rotor scale $\theta \sim c / (2\pi r_\text{comp})$ would test the extra phase-lock structure, not the dual-only geometry of $J$.
\end{itemize}
