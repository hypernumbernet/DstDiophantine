...
\end{equation}
A constant drive shifts the equilibrium mismatch by \(-u/(2m)\) (when \(m\neq 0\)) while the common rapidity accelerates. Resonance is therefore additional structure, not an identity of either quadratic density. Identifying \(u\) with a helicity-odd Faraday bilinear, such as the Poynting component of a circular wave, is a modelling choice. Among the six classical Faraday quadratics that bilinear is the unique helicity-odd combination on circular waves; it is still not a derivation of a coupling to the particle mismatch.
