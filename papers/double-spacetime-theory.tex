...
Gravity is the energetic cost of dual-rotor misalignment. Because $J$ depends solely on the relative rotor $\Omega$, dual-only modulation of $\theta$ cannot raise $J$. It conserves $J+M$, and dual-only shielding is possible precisely when the dual wall is nonpositive; a strictly negative wall admits repulsion. Gravitational neutrality is moreover bounded in mass: no admissible configuration with $J=0$ carries more than five sixths of the cone's mass ceiling, whereas full repulsion reaches that ceiling unimpeded. The same pairing that caps a shield is not the global envelope of the cone. Outside $|J|\le\pi^2/8$ a tighter bound takes over, and the exact $(J,M)$ domain is a three-lobed region whose outer peaks again meet the mass ceiling. Whether an external field supplies that dual-channel force is a conjecture, taken up in Section~\ref{sec:gravitycontrol}. The continuum remains a macroscopic description; spacetime degrees of freedom in this theory are particle-local and paired.
...
An electromagnetic coupling, a resonance frequency, or a laboratory protocol is not forced by this geometry. The remainder of this section records a proposed model for a dual-channel force, not a derivation of that force from Faraday coefficients.
...
The circular wave recorded there is null at every phase, and every linear Faraday coefficient has vanishing four-phase mean, so
\[
\langle F_{\mathrm{usual}}\rangle=\langle F_{\mathrm{dual}}\rangle=0.
\]
There is in particular no mean dual field proportional to $\sigma E_0^2 K$. What is helicity-odd and constant is the bilinear Poynting component $(E\times B)_z=\sigma E_0^2$ of that section. Linear polarisation carries a nonnegative Poynting component and no helicity sign. Superposition of the wave onto a background torsion does not shift the mean of $J$, and the Faraday mix cannot create $J$ from the wave. A first-order rest sandwich of the dual summand vanishes for every Faraday field, circular waves included. The rest sandwich of the wave itself is the oscillating electric kick, whose four-phase mean vanishes; a velocity along the beam produces a transverse kick with no longitudinal component~\eqref{eq:faraday-lorentz}. Axis projectors $P_{L,R}$ likewise fail to extract helicity.
...
where the dual generators $\Gamma_a$ mediate the interaction. The phase-lock
\[
