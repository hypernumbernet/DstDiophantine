...
\]
Dual-only $J$ cannot fall below the wall. Shielding $J=0$ by dual excitation alone is possible if and only if the wall is nonpositive, equivalently $\sum_a\alpha_a\le 3\pi/4$. Repulsion $J<0$ requires a strictly negative wall. The balanced target $\beta=\alpha$ lies in the cone precisely when $\alpha_a\le\pi/4$ on every axis: a strongly boosted usual sector cannot be dual-only shielded; the usual rotor must be unwound as well. Uniform seeds make the dichotomy sharp. At $\alpha=\pi/6$ the equal-scale locus is admissible and massive with $J=0$, and the wall is repulsive. At $\alpha=\pi/3$ no dual-only configuration reaches $J=0$.

The same six numbers that define $J$ also define the unsigned mass $M=\tfrac12\sum_a(\alpha_a^2+\beta_a^2)$. Independently of any trajectory,
\[
J+M=\sum_a\alpha_a^2,\qquad M-J=\sum_a\beta_a^2.
\]
Dual-only motion therefore travels on the line $J+M=\mathrm{const}$. Every reduction of the gravitational mismatch is paid for, one-for-one, by unsigned mass. From a pure usual seed ($\beta=0$) one has $J=M$, so every dual-only shield lands at $M=2M_{\mathrm{seed}}$. Because $J=0$ is $\sum\beta^2=\sum\alpha^2$, that dual budget is unique: admissible shields from a given usual seed differ only in how the budget is shared among axes. Dual-only $J$ fills the closed interval from the wall up to the pure-usual value. Nothing in between is missing, and nothing beyond the wall is reachable without moving $\alpha$.

Usual-only motion conserves $M-J$. When the wall is positive, dual excitation cannot cancel $J$, but unwinding the usual rotor lowers the conserved ceiling $\sum\alpha^2$ itself. Truncating each $\alpha_a$ to at most $\pi/4$ and matching the dual angle always yields a massive equal-scale shield. At $\alpha=\pi/3$ that mixed unwind succeeds where dual-only fails. Inertial resistance, in the sense of Section~\ref{sec:unified}, tracks the mismatch $J$, not the unsigned mass $M$. Synchronising the dual rotor with the usual one can null $J$ while increasing $M$; reducing both requires walking the equal-scale locus toward the origin.
...
If a dual-channel generalised acceleration $u$ is added to $\ddot\theta$ in the written particle action of Section~\ref{sec:dualrotordynamics}, the channels become
\[
\ddot\delta=-u,\qquad \ddot\sigma=-2m\delta+u.
\]
The mismatch is a double integrator: no restoring frequency stands between a dual force and a change of $J$. Those identities assume both channels free. Holding the usual rapidity fixed, $\ddot\phi=0$, forces $m(\phi-\theta)=0$: a leftover mismatch with nonzero rest mass drags the usual sector. Dual-only kinematics is therefore not a free solution of the written action. Realising it requires a constraint on $\phi$, or else both rotors move and the mismatch remains the double integrator already written. The neighbouring same-sign oscillator is no exception---$\ddot\phi=0$ again forces the potential to vanish---and instead obeys $\ddot\delta+2m\delta=-u$, so a constant drive shifts the equilibrium mismatch while accelerating the common rapidity. Resonance is therefore additional structure, not an identity of the written action.
...
A dual-channel generalised acceleration $u$ added to $\ddot\theta$ does not restore a frequency in the written action: $\ddot\delta=-u$ and $\ddot\sigma=-2m\delta+u$. Holding $\ddot\phi=0$ forces $m(\phi-\theta)=0$, so dual-only motion is not a free solution. The neighbouring oscillator instead obeys $\ddot\delta+2m\delta=-u$. Whether laboratory fields supply such a $u$ is the modelling question of Section~\ref{sec:gravitycontrol}, not a consequence of these identities.
