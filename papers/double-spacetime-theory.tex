...
Section~\ref{sec:unification} records the Faraday 6-space, the distinction between the dual map and axis chirality, the $3{+}3$ split of Faraday generators relative to $e_1$, the same-projector sandwich selection rule, the one-parameter mix of $(E,B)$, the circular-wave identities of $J$, Poynting, and superposition, the fact that axis chirality is not photon helicity, and the identity of the Faraday sandwich on $e_0+v$ with the Lorentz 4-force of $(E,-B)$. Gauge identifications and a Weinberg angle are not derived.
...
J(F)=\tfrac12(E^2-B^2),
\]
and the unsigned mass is $M(F)=\tfrac12(E^2+B^2)$. Fields with $E^2=B^2$ have $J=0$. A circularly polarised wave of helicity $\sigma=\pm 1$ (units $c=1$), at every phase $\psi=kz-\omega t$,
\[
\mathbf{E}=E_0(\hat{x}\cos\psi+\sigma\hat{y}\sin\psi),\qquad
\mathbf{B}=E_0(-\hat{x}\sin\psi+\sigma\hat{y}\cos\psi),
\]
is likewise null: $J=0$, $E\cdot B=0$, and $M=E_0^2$ whenever $E_0\neq 0$. The circular snapshot is the special case $\psi=0$. A null Faraday field occupies the balanced massive locus, not the vacuum $M=0$.
\end{proposition}
...
\begin{proposition}[Helicity Poynting of a circular wave]
\label{prop:circular-poynting}
For the circular wave of Proposition~\ref{prop:faraday-quadratic} the signed Poynting component along the propagation axis equals $\sigma E_0^2$ at every phase. Laboratory time reversal reverses that sign; duality $(E,B)\mapsto(B,-E)$ preserves it. A linearly polarised travelling wave $E=E_0\cos\psi\,\hat{x}$, $B=E_0\cos\psi\,\hat{y}$ is also null, but its Poynting component is $E_0^2\cos^2\psi\ge 0$ and carries no helicity sign, while $M$ oscillates as $E_0^2\cos^2\psi$. The two local Faraday invariants $J$ and $E\cdot B$ vanish identically on the circular wave, so a helicity-odd scalar cannot be a polynomial in those invariants.
\end{proposition}
...
Write $F_{\parallel}$ and $F_{\perp}$ in Faraday coefficients as the Cartan triple $(B_1,E_2,E_3)$ and the charged triple $(E_1,B_2,B_3)$.

\begin{proposition}[Axis chirality is not photon helicity]
\label{prop:axis-not-helicity}
Relative to $e_1$, the circular wave of Proposition~\ref{prop:faraday-quadratic} is the phase quadrature of those two triples:
\[
\mathbf{E}_{\parallel}=E_0\sigma\sin\psi\,\hat{y},\qquad
\mathbf{B}_{\parallel}=-E_0\sin\psi\,\hat{x},
\]
\[
\mathbf{E}_{\perp}=E_0\cos\psi\,\hat{x},\qquad
\mathbf{B}_{\perp}=E_0\sigma\cos\psi\,\hat{y}.
\]
Each summand is null. The signed Poynting splits without a cross term,
\[
(E\times B)_z\big|_{\parallel}=\sigma E_0^2\sin^2\psi,\qquad
(E\times B)_z\big|_{\perp}=\sigma E_0^2\cos^2\psi.
\]
Both sandwiches $P_R F P_R$ and $P_L F P_L$ annihilate the charged summand, leaving only a linearly polarised Cartan field. At the snapshot phase $\psi=0$ that Cartan field vanishes, so $P_R F P_R=0$. At $\psi=\pi/2$ the wave is purely Cartan. Neither projector selects the helicity sign $\sigma=\pm 1$: a circular wave is not an eigenstate of axis chirality.
\end{proposition}
...
Componentwise superposition of Faraday fields is bilinear in the coefficients.

\begin{proposition}[Superposition and mix of a circular wave]
\label{prop:circular-superposition}
If $p$ and $q$ are Faraday configurations, then
\[
J(p+q)=J(p)+J(q)+(\mathbf{E}_p\cdot\mathbf{E}_q-\mathbf{B}_p\cdot\mathbf{B}_q).
\]
Superposing the circular wave of Proposition~\ref{prop:faraday-quadratic} onto a background therefore shifts $J$ by a first harmonic in the phase. The four-phase mean of $J$ is the background value: linear superposition does not displace mean mismatch. The Faraday mix of Proposition~\ref{prop:faraday-mix} applied to that circular wave still yields $J=0$.
\end{proposition}

The first-order sandwich increment of $R=1+\varepsilon\Omega$ is the commutator $\Omega X-X\Omega$. On a rest particle a pure electric generator produces a spatial kick and every magnetic generator produces none, so the dual Faraday summand never kicks a rest frame. On a particle with velocity along $y$, a pure $B_x$ generator produces a kick along $z$. The outer product $F\wedge X$ is nonzero on the magnetic rest case and vanishes on that moving magnetic case.

\begin{proposition}[Sandwich increment versus the written wedge]
\label{prop:faraday-sandwich}
The first-order increment of $RX\widetilde R$ is the commutator, not the outer product. A rest electric generator and a moving magnetic generator each produce a spatial kick; every rest magnetic generator produces none, so the dual Faraday summand of an arbitrary field---a circular wave included---vanishes on a rest frame at first order. In particular $F\wedge X$ is not the Lorentz increment of the written sandwich.
\end{proposition}

Write $X=e_0+\mathbf{v}$ for the non-relativistic Minkowski vector of a particle.

\begin{proposition}[Sandwich increment is the Lorentz 4-force of $(E,-B)$]
\label{prop:faraday-lorentz}
The commutator of the Faraday bivector on $X$ is the 4-vector
\[
FX-XF=(E\cdot v)\,e_0+(E-v\times B).
\]
That is the Lorentz 4-force of the laboratory time-reversed field $(E,-B)$. The coefficient 3-force $q(E+v\times B)$ differs from the sandwich by the sign of $B$; laboratory time reversal converts one into the other. A rest frame therefore receives the electric kick $E$ with vanishing power. A velocity along the beam of the circular wave of Proposition~\ref{prop:faraday-quadratic} yields a transverse kick with no longitudinal component; the four-phase mean of the rest electric kick vanishes. Maxwell's equations are not derived: the identity is kinematic on the written sandwich.
\end{proposition}

The algebraic relation between the electromagnetic and weak channels is therefore this: both live in the Faraday 6-space; the dual map exchanges the two sectors and is not a chirality projector; an axis $e_1$ splits the six generators into a commuting triple and an anticommuting triple, of which only the former survives the same-projector sandwich; a circular wave is the phase quadrature of those triples, so the sandwich destroys circular polarisation and does not select helicity; same-axis $E$ and $B$ mix by a single angle whose quadratic law contains duality as a special value; the sign of $J$ distinguishes the noncompact electric sector from the compact magnetic sector; and the Faraday sandwich on a particle's 4-velocity is the Lorentz 4-force of $(E,-B)$, not of $(E,B)$.
...
At every phase that wave is null: $J=0$, $E\cdot B=0$, and $M=E_0^2$ whenever $E_0\neq 0$ (Proposition~\ref{prop:faraday-quadratic}). Each linear Faraday coefficient is a first harmonic of the phase, so the four-phase means vanish,
\[
\langle F_{\mathrm{usual}}\rangle=\langle F_{\mathrm{dual}}\rangle=0.
\]
There is in particular no mean dual field proportional to $\sigma E_0^2 K$. What is helicity-odd and constant is the bilinear Poynting component $(E\times B)_z=\sigma E_0^2$ (Proposition~\ref{prop:circular-poynting}). Superposition of the wave onto a background torsion does not shift the mean of $J$, and the Faraday mix cannot create $J$ from the wave (Proposition~\ref{prop:circular-superposition}). A first-order rest sandwich of the dual summand vanishes for every Faraday field, circular waves included (Proposition~\ref{prop:faraday-sandwich}). The rest sandwich of the wave itself is the oscillating electric kick, whose four-phase mean vanishes; a velocity along the beam produces a transverse kick $E-v\times B$ with no longitudinal component (Proposition~\ref{prop:faraday-lorentz}). Axis projectors $P_{L,R}$ likewise fail to extract helicity: they annihilate the charged summand of the wave and kill the circular snapshot outright (Proposition~\ref{prop:axis-not-helicity}).

A helicity-dependent drive of the particle mismatch is therefore not a local scalar of $F$, nor a linear superposition on the Faraday 6-space, nor a first-order rest sandwich of $F_{\mathrm{dual}}$, nor an axis-chirality projection, nor a beam-direction Lorentz kick of the particle's 4-velocity. Any such drive requires extra structure---a preferred axis, a potential, or the particle's dual phase---and belongs to Hypothesis~\ref{hyp:coherent-excitation}. In that working model the helicity-odd bilinear $(E\times B)_z=\sigma E_0^2$ is the candidate source of a dual-phase drive, with right-handed polarisation ($\sigma=+1$) reinforcing attractive mismatch ($J>0$) and left-handed polarisation ($\sigma=-1$) opposing it.
...
\item Faraday coefficients $(E,B)$ occupy the same six-generator space as the dual rotor, with the split $F_{\mathrm{usual}}\leftrightarrow\mathbf{E}$, $F_{\mathrm{dual}}\leftrightarrow\mathbf{B}$. On that identification $J=\tfrac12(E^2-B^2)$ and $M=\tfrac12(E^2+B^2)$. A circularly polarised wave of helicity $\pm 1$ is null at every phase ($J=0$, $E\cdot B=0$) yet massive, with constant signed Poynting $(E\times B)_z=\sigma E_0^2$; laboratory $T$ reverses that sign and duality preserves it. Linear polarisation is also null but carries no helicity sign. Linear superposition of a circular wave onto a background does not shift mean $J$; the Faraday mix cannot create $J$ from the wave; the dual Faraday summand never kicks a rest frame at first order. Duality $(E,B)\mapsto(B,-E)$ has period 4 and sends $J\mapsto -J$; the usual--dual swap also sends $J\mapsto -J$ but has period 2; laboratory $T$ preserves $J$. A one-parameter mix rotating $(E,B)$ acts by $J\mapsto J\cos 2\omega+(E\cdot B)\sin 2\omega$, of which duality is $\omega=\pi/2$. Pure electric configurations have $J\ge 0$; pure magnetic ones have $J\le 0$. The dual map is not a chirality projector: $(1\pm i)/2$ is not idempotent, while $P_{L,R}=(1\pm e_1)/2$ are complementary idempotents, and duality does not preserve the axis $e_1$ as a real line. Relative to $e_1$ the six Faraday generators split $3{+}3$; the same-projector sandwich $P_R X P_R$ retains only the commuting summand. A circular wave is the phase quadrature of those triples, each null, with Poynting parts adding without a cross term; both $P_{L,R}$ kill the charged summand, so axis chirality is not photon helicity. The first-order sandwich increment of the Faraday bivector on $e_0+v$ is the 4-vector $(E\cdot v)\,e_0+(E-v\times B)$, the Lorentz 4-force of laboratory time-reversed $(E,-B)$; the coefficient 3-force $q(E+v\times B)$ differs by the sign of $B$. A rest frame receives the electric kick $E$; a beam-direction velocity of a circular wave yields a transverse kick. The outer product $F\wedge X$ is not the Lorentz increment.
\end{itemize}
