...
Section~\ref{sec:darkmatter}, after the unification of the gravitational and electromagnetic channels, records the three-sphere chart: Gauss flux, the circular-speed identity \(v^2=(GM/R)f\), the unique minimum of \(f\) that produces a plateau, and the fact that attraction ends at \(\pi R/2\), strictly before \(r=2R\). That galaxies are such patches, and that this compactness therefore accounts for flat rotation curves without dark matter, remains a working identification.
...
Section~\ref{sec:conclusions} returns to the ontology of the theory, distinguishes what the algebra has established from what remains hypothesis, and names the observations by which the continuum reading may be tested.
...
Having established the layered torsional geometry of gravity and electromagnetism at nuclear and atomic scales (Sections~\ref{sec:strongfield}--\ref{sec:electronshells}) and their unification as two channels of a single dual-rotor mechanism (Section~\ref{sec:unification}), we now examine how each channel spatially unfolds at cosmological scales. The gravitational channel, driven by the kinematic pair $(\phi, \theta)$ tied to inertial mass, is read as compactifying the dual sector into galaxy-scale $S^3$ patches; the electromagnetic channel, driven by the independent pair $(\alpha, \beta)$ tied to charge, unfolds over substantially larger, near-cosmic scales. That reading is a working identification. Of the three-sphere chart itself the following are theorems: the cotangent potential obeys Gauss's law; circular speed is $v^2=(GM/R)\,f(r/R)$; the shape $f$ has a unique minimum on the attractive interval; and attraction ends at $r=\pi R/2$, strictly before $r=2R$. If galaxies are such patches, the systematic distortion of the compact-to-flat map is what observers interpret as missing gravitational mass, and flat rotation curves emerge from baryons alone. That physical step is not a theorem of the dual-rotor algebra.
...
\label{prop:s3-gauss}
The radial derivative of the cotangent potential is $-g(r)$, and the flux through geodesic spheres is constant:
\[
g(r)\cdot 4\pi R^2\sin^2(r/R)=-4\pi GM.
\]
Consequently $\Phi$ is radially harmonic away from the origin. On the first hemisphere $0<r<\pi R$, one has $\Phi(r)=0$ if and only if $r=\pi R/2$.
\end{proposition}
...
\begin{proposition}[Circular speed on $S^3$]
\label{prop:s3-circular-speed}
...
\]
where $f(x)=x/\sin^2 x$. Flatness of $v$ is constancy of $f$.
\end{proposition}

Outside the luminous disk ($r > r_{\text{disk}}$), where $M(r) \approx M_{\text{tot}}$, the flat-space Keplerian factor $GM_{\text{tot}}/r$ declines as $r^{-1}$. The strictly increasing enhancement $\eta$ counteracts this decline. The combined shape is $f$ itself.

\begin{proposition}[Unique minimum of the rotation-curve shape]
\label{prop:plateau-root}
On the attractive interval $(0,\pi/2)$ the shape $f(x)=x/\sin^2 x$ has a unique critical point, the unique root $x_0$ of $\tan x=2x$ in $(57/50,\,6/5)=(1.14,1.20)$. At that root $f(x_0)=x_0+1/(4x_0)$, and this value $f_0$ is the unique minimum of $f$ on $(0,\pi/2)$:
\[
\frac{135}{100}<f_0<\frac{141}{100}.
\]
The decimal $f_0\approx 1.38$ is only a reading of that window. Equivalently, $f'(x)=0$ if and only if $\tan x=2x$.
\end{proposition}

Proposition~\ref{prop:plateau-root} is the geometric origin of a plateau: after the inner Keplerian decline is arrested, $v^2$ sits near $(GM/R)f_0$ throughout a neighbourhood of $r=x_0 R$. Earlier drafts placed an ``outer rise'' at $r\gtrsim 2R$ and a reversal at $\pi R/2$. That ordering is impossible. One has $x_0<\pi/2<2$, so the attractive patch ends at $r=\pi R/2$, strictly before $r=2R$. The three regions on a point-mass chart are therefore:
...
\]
where $f_0$ is the unique minimum of Proposition~\ref{prop:plateau-root}. If the compactification radius scales with the total enclosed mass as
\[
...
\]
of which $R\approx 8.3\,\mathrm{kpc}$ is only a reading of the window. The geometric plateau therefore sits at $x_0 R$, in $(9,11)\,\mathrm{kpc}$, inside the optical disk, while the torsional reversal at $\pi R/2$ lies in $(12,15)\,\mathrm{kpc}$. Identifying $a_0$ with a characteristic acceleration on the $S^3$ patch remains a working hypothesis.
...
Of the algebra, and of the charts constructed from it, the following stand as theorems. The relative rotor is the identity precisely when the two rotors coincide. A unique Lorentz-invariant scalar $J$ measures the mismatch; it is bounded on the admissible cone, and vanishing $J$ does not force the rotors to agree. On every static radial-boost gauge, $J$ stands in a closed dictionary with the Weitzenb\"ock torsion $T$ of teleparallel gravity, of which the exterior Schwarzschild geometry is the unique vacuum specialisation. The two agree only in the flat limit. A real radial boost cannot produce repulsion; repulsive layers belong to the elliptic sector and are of bounded depth. Where the two rotors share a common scale, $J$ vanishes while their interference still alternates, so the layers are ordered by that interference and not by $J$. The same algebra houses the electromagnetic channel: Faraday coefficients occupy the six-generator space of the dual rotor. The written particle action has Euler--Lagrange equations; the dual rotor is an element of $\mathrm{SU}(2)$. On the three-sphere chart the circular speed is $v^2=(GM/R)f(r/R)$, the shape $f$ has a unique minimum on $(0,\pi/2)$, and attraction ends at $\pi R/2$, before $r=2R$. None of these identities requires a background manifold.

Of physics, the following remain working hypotheses, and none is a theorem of the algebra: that the layered phase is the strong force; that galaxies are three-sphere patches of the dual sector, and that this compactness therefore accounts for flat rotation curves; that coherent excitation of the dual sector might engineer gravity; that early dual dynamics sources baryon asymmetry; that Maxwell's equations, a Weinberg angle, and a Higgs reading of rotor rigidity can be recovered from the same six-space; that the mismatch oscillator is the de Broglie phase; and that a Hilbert-space quantization of the dual rotor completes the theory. Uncertainty relations are not derived. Controllability of gravity is a conjecture, not an identity.

What can already be asked of the sky, without those open derivations, is modest and sharp. In place of a true horizon one expects a finite redshift floor. At the centre of a spherically symmetric torsion star, gravitational time dilation should vanish. If a galaxy is a three-sphere patch, its rotation curve plateaus near $r\sim x_0 R$ with $x_0\in(1.14,1.20)$ and reverses at $\pi R/2$, rather than by unseen mass; for Milky-Way stand-in mass the plateau sits near $9$--$11\,\mathrm{kpc}$ and the reversal near $12$--$15\,\mathrm{kpc}$. The ratio of successive barrier radii is a pure number, the same at nuclear and at stellar scales, because the characteristic length cancels from it.
