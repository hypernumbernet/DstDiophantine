...
These families are the whole of the envelope. An admissible configuration meets it precisely when every axis lies on the wall and at least two axes are purely usual or purely dual. In the open central band the locked pair is opposite in sector, and $J$ fixes the split of the remaining axis, so each assignment of the three roles is one configuration; the even split among them is the heaviest shield. On an open outer arc the locked pair belongs to one sector, and $J$ again fixes the rest. Nothing on this boundary has an internal modulus. Dual-only motion holds the usual rapidities fixed, and the wall then determines the dual angles, so that motion meets the envelope at most once. Sliding along the ceiling, from a light shield toward a two-axis lock or across neutrality, has to move the usual rotor.
