...
The electromagnetic and weak channels therefore live in one six-space. The dual map exchanges the two sectors and is not a chirality projector. The sign of $J$ distinguishes the noncompact electric sector from the compact magnetic sector. The sandwich on a particle's 4-velocity is the Lorentz force of $(E,-B)$. A mix $\omega$ rotating $(E,B)$ on each axis is the duality circle, and at $\omega=\pi/2$ it swaps the Cartan $\mathfrak{so}(2,1)$ with the non-closed charged complement: it does not rotate two compact $\mathrm{SU}(2)$ factors into each other. A dual-only increment of the dual angles occupies that six-space: superposing a magnetic Faraday field, and substituting $\theta_a\mapsto\theta_a+e A_a$ in the dual rotor, are two laboratory readings of it. Their kinematics occupy Section~\ref{sec:gravitycontrol}. An extended rotor built from $\int F\,ds$, a Weinberg angle read from the duality mix, and a Higgs reading of rotor rigidity remain open. The dual map does not imply a V--A coupling.
