...
\]
so an axis can be heavy only by being lopsided. Nothing forbids three axes from being lopsided in opposite senses, and that is precisely the freedom shielding exploits; but the cancellation is itself bounded, because each axis density $\alpha_a^2-\beta_a^2$ lies in $[-k^2,k^2]$, and three such numbers cannot drive the sum of their pairwise products below $-k^4$. Assembling the two facts binds the two observables of the theory in one sharp inequality,
\begin{equation}
...
Read at neutrality \eqref{eq:neutral-def}, the inequality \eqref{eq:shield-ceiling} says that a shielded particle cannot weigh more than
\[
M\le\frac{5\pi^2}{16}=\frac56\cdot\frac{3\pi^2}{8},
\]
five sixths of the cone's own mass ceiling. The remaining sixth of the available mass is closed to neutrality---whatever trajectory is taken, and whatever force drives the dual channel. Nor is the bound an artefact of the estimate: it is attained, and attained along an entire one-parameter family lying on the wall,
\[
\alpha=(\pi/2,\,0,\,\gamma),\qquad\beta=(0,\,\pi/2,\,\pi/2-\gamma),\qquad 0\le\gamma\le\pi/2,
\]
for which $J=\tfrac\pi2(\gamma-\pi/4)$ and $M=\tfrac{5\pi^2}{16}+(\gamma-\pi/4)^2$ turn \eqref{eq:shield-ceiling} into an identity. The heaviest shield in the theory is the midpoint $\gamma=\pi/4$: one axis purely usual, one axis purely dual, the third split evenly, all three on the wall.
...
At the top of the mass range the freedom vanishes altogether. A configuration attaining $M=3\pi^2/8$ must have every axis either purely usual, $(\alpha_a,\beta_a)=(\pi/2,0)$, or purely dual, $(0,\pi/2)$; at maximal radius the cone admits no intermediate. Such a configuration's mismatch is a sum of three quanta $\pm\pi^2/8$, so $|J_{\mathrm{norm}}|\ge 1/3$. The heaviest configurations the theory admits are not merely hard to shield: they are not gravitationally neutral at all, and nothing can make them so without first making them lighter.

The asymmetry between the two directions deserves saying plainly, since it inverts the intuition that repulsion should be the expensive state. The ceiling in \eqref{eq:shield-ceiling} is a ceiling on neutrality, not on gravity. Full repulsion costs nothing: the pure-elliptic configuration $\alpha=0$, $\beta_a=\pi/2$ carries the maximal mass $3\pi^2/8$ together with the extreme value $J_{\mathrm{norm}}=-1$, and its attractive mirror does the same. Attraction and repulsion are available at every weight; what is obstructed is the crossing between them, and the obstruction grows quadratically in how close to zero one insists on standing. The design constraint of a control programme is therefore not how much dual excitation can be supplied, but how heavy an object one is willing to leave gravitationally visible.
...
If the model is right, gravity is not merely controllable but chirally controllable, with helicity serving as the switch between attraction and repulsion. Dual-only kinematics already say when shielding and repulsion are reachable on the admissible cone, and at what unsigned-mass cost. Whether laboratory electromagnetism supplies the dual force is a conjecture, to be decided in the laboratory.
