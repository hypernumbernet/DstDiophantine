...
\newcommand{\Mnorm}{M_{\mathrm{norm}}}
...
Three vanishing conditions must not be conflated. If $M=0$, then $J=0$: every rapidity vanishes, and this is vacuum. The converse is false. Equal usual and dual rapidities give $J=0$ with $M>0$---balanced massive states---and on a single axis those states still have $\Omega\neq 1$. Thus $\Omega=1$, $J=0$, and $M=0$ are three different physical situations. Gravity as free fall is $\Omega=1$, not the vanishing of $J$. On one axis, with $\sigma$ the sum of the rapidities and $\delta$ their difference, $J=\frac12\delta\sigma$ and $M=\frac14(\sigma^2+\delta^2)$: vanishing $\delta$ is the balanced locus, and it is the vacuum only when $\sigma$ vanishes as well.
...
Gravity is the energetic cost of dual-rotor misalignment. Because $J$ depends solely on the relative rotor $\Omega$, dual-only modulation of $\theta$ cannot raise $J$. It conserves $J+M$, and dual-only shielding is possible precisely when the dual wall is nonpositive; a strictly negative wall admits repulsion. Gravitational neutrality is moreover bounded in mass: no admissible configuration with $J=0$ carries more than five sixths of the cone's mass ceiling, whereas full repulsion reaches that ceiling unimpeded. The exact $(J,M)$ image of the cone is the three-lobed domain of Section~\ref{sec:gravitycontrol}. On a boosted axis the clock factor $\gamma_{\mathrm{eff}}$ falls with dual excitation and meets $1$ before the coaxial shield; a cross-axis shield cancels $J$ while leaving that clock at $\cosh\alpha$. Whether an external field supplies that dual-channel force is a conjecture, taken up in that section. The continuum remains a macroscopic description; spacetime degrees of freedom in this theory are particle-local and paired.
...
\item Synchronising the dual rotor $\theta$ with $\phi$ can cancel the mismatch $J$ that is felt as inertial resistance. Dual-only, that cancellation is purchased one-for-one by unsigned mass $M$: it is not a reduction of $M$, and the dual budget may be parked on a silent axis. Reducing both $J$ and $M$ requires unwinding the two rotors together, toward the origin. The cancellation is also capped: a configuration with $J=0$ cannot carry more than five sixths of the maximal unsigned mass, so a sufficiently heavy body must be made lighter before its inertial signature can be cancelled at all. Either operation is a kinematic possibility on the cone, not yet a laboratory coupling---a direct pathway, if a dual-channel force exists, toward propellantless control of inertia.
...
This $\theta$-centric formulation makes manifest that time dilation is fundamentally a torsional phenomenon originating in the particle-intrinsic dual spacetime. The usual rapidity $\phi$ provides the kinematic baseline, while the compact dual angle $\theta$ modulates that baseline. Those two displacements do not return together: the clock factor meets $1$ at a dual angle strictly before the coaxial shield, and a cross-axis shield that cancels $J$ leaves the boosted clock at its special-relativistic value. External coherent excitation of $\theta$ therefore acts on time dilation and on gravitational mismatch as two distinct kinematic coordinates.
...
\]
which decreases in \(n\) for \(n\ge 1\). What grows inward without bound is the barrier structure---the nodes at which \(\gamma_s\) vanishes and \(1/\gamma_s\) diverges. The derivative \(-2\cosh x\sin x\) does not vanish on \((0,\pi)\), so each zero is simple: the inward force has a simple pole, and the radial potential falls logarithmically, without a lower bound. The plateaux between barriers become exponentially quiet, successive factors falling like \(\mathrm{e}^{-n\pi}\). In radius the screening is \(1/\cosh\bigl(\ell/(2r)\bigr)\): the deep interior of a torsional condensate is quasi-free between the poles. The poles interrupt a smooth crossing. Reading that interruption as nuclear confinement remains a proposal.
...
\item Deeper layers: strictly alternating attraction and repulsion, one barrier per \(\pi\)-interval of \(x\), with exponentially quieter plateaux and a nearly free interior between the poles.
\end{itemize}
...
The force is undefined at a node. A node is a force divergence, not a clock freeze: \(\gamma_{\mathrm{eff}}=\gamma_b^2>0\) by~\eqref{eq:gamma-eff}. The same simple zero lets the radial potential fall without a lower bound, and the divergence does not by itself supply a circular orbit.
...
\section{Stability on the balanced locus}
\label{sec:particle-stability}

On a configuration with vanishing mismatch and positive unsigned mass the two rapidity budgets are equal, and each is the mass itself,
\[
\sum_a\alpha_a^2=\sum_a\beta_a^2=M.
\]
A motion that holds either budget fixed cannot reach the vacuum. Fixing the usual rapidities conserves \(J+M\); fixing the dual angles conserves \(M-J\). In either case a positive budget remains, and with it a positive mass. The configuration of vanishing mass differs from such a seed in both \(\alpha\) and \(\beta\).

The written one-axis flow has the same rest point. Vanishing mismatch, vanishing mismatch rate, and vanishing common velocity keep both angles fixed, so the unsigned mass does not move. A nonzero mismatch rate leaves that value at once and is not restored, because the written density frees \(\delta\). On the aligned locus the same flow keeps the mismatch at zero, and the unsigned mass is one quarter of the square of the common rapidity. That mass stands still if and only if the common rate vanishes. A nonzero common rate meets the vacuum once and does not stay there: except at that instant the two rotors still disagree. Alignment holds the mismatch. It does not hold the mass.

The hydrogen well does not pay this rest. Its ionisation energy is exactly \(\alpha^2/2\) of the electron rest energy, and that fraction lies between two and three parts in one hundred thousand, so the rest energy is more than \(3\times 10^4\) hydrogen ionisations and fewer than \(5\times 10^4\). The exterior Coulomb layer can bind the electron. It cannot account for the electron.

Nor does the first node. The substitution \(x=\lambda/(2r)\) turns the inward change of the radial potential into \(-2ke^2/\lambda\) times \(\int dx/\gamma_s\), and that integral is unbounded as \(x\) approaches the node. A circular orbit in the attractive well still has no continuation through it: on the repulsive side the written \(v^2\) is negative, and at the node the force is undefined. What holds the electron is the equality of the two budgets, which one channel cannot spend, together with the smallness of this binding against the rest energy.
...
\]
is idempotent, sums to the unit, and multiplies to zero. Duality sends $e_1$ to the trivector $-e_0 e_2 e_3$, which is not a real multiple of $e_1$: the chirality axis is not a dual-invariant line. The product of the commuting square-$+1$ pair $e_1$ and $e_0e_2$ generates a left ideal of real dimension eight. That ideal is the direct sum of the four-dimensional Minkowski core $\mathrm{Cl}(3,1)\,P$ and a copy along the null generator $e_4$, hence reducible as a $G(3,1,1)$-module; we do not identify it with an irreducible Dirac module, nor claim $\mathrm{Cl}(3,1)$ irreducibility of the core.
...
Relative to a spatial axis $e_a$ the six generators split into a commuting triple $(B_a,E_{a+1},E_{a+2})$ and an anticommuting triple $(E_a,B_{a+1},B_{a+2})$. The sandwich $P_R X P_R$ keeps the former and kills the latter; the selection is axis-dependent, not a Lorentz-scalar Weyl projection. The commuting triple is a Lie algebra: its brackets close, with the parallel magnetic generator compact and the two perpendicular electrics noncompact, so the algebra is $\mathfrak{so}(2,1)$ rather than $\mathfrak{su}(2)$. The anticommuting complement does not close; the bracket of its first two generators is the remaining Cartan electric. Duality exchanges the two triples, up to signs. Among the six written generators the only triple in which every element squares to $-1$ is the dual-rotor (magnetic) triple itself, and those three satisfy the quaternion table $IJ=K$. Compact $\mathrm{SU}(2)$ in this six-space is therefore dual-sector rotation, not a chirality-selected Faraday Cartan. Three spatial axes each furnish such a split, and the associated projectors are complementary and idempotent, but projectors of distinct axes do not commute, nor do the three right projectors sum to the unit: they are alternative embeddings of the same $3+3$ geometry, not three simultaneous generation labels. The time axis squares to $-1$ and cannot serve as a chirality generator, for the same algebraic reason as the pseudoscalar. A circular wave is the phase quadrature of those two triples, so neither projector selects the helicity sign $\sigma=\pm 1$. Axis chirality is not photon helicity.

The commuting triple is the stabilizer of the axis. Rotation about the axis, and a boost perpendicular to it, leave $e_a$ fixed. A dual rotation by an angle $\theta$ about the perpendicular generator $B_{a+1}$ turns the axis in a spatial plane,
\[
e_a \mapsto \cos\theta\, e_a - \sin\theta\, e_{a+2},
\]
and the rotation about $B_{a+2}$ turns it toward the remaining direction,
\[
e_a \mapsto \cos\theta\, e_a + \sin\theta\, e_{a+1}.
\]
At $\theta=\pi$ the axis has reversed, and $P_L$ is exchanged with $P_R$. Of the three compact generators only $B_a$, the rotation about the axis itself, holds the axis still. The dual rotor moves handedness. It is not a gauge symmetry of a fixed projector.

A boost of rapidity $\alpha$ along the axis does not turn the axis into another spatial direction. It tilts the generator into time,
\[
e_a \mapsto \cosh\alpha\, e_a + \sinh\alpha\, e_0,
\]
and the image still squares to $+1$. The parallel electric generator is not itself a rotor: multiplied by its reverse it equals $-1$, while each magnetic generator is a rotor. What a fixed spatial axis selects inside this six-space is therefore the noncompact stabilizer $\mathfrak{so}(2,1)$, and the only compact direction in that stabilizer is the single generator $B_a$.

In the plane spanned by that generator and the parallel electric field, an element $c_B B_a + c_E E_a$ commutes with $e_a$ if and only if $c_E=0$. A duality mix of the pair leaves the stabilizer at every nontrivial angle. There is no chirality-preserving Weinberg angle in this plane. The stabilizer contains no second compact generator with which $B_a$ could mix while holding the axis fixed.
...
The electromagnetic and weak channels therefore live in one six-space. The dual map exchanges the two sectors and is not a chirality projector. The sign of $J$ distinguishes the noncompact electric sector from the compact magnetic sector. The sandwich on a particle's 4-velocity is the Lorentz force of $(E,-B)$. A mix $\omega$ rotating $(E,B)$ on each axis is the duality circle, and at $\omega=\pi/2$ it swaps the Cartan $\mathfrak{so}(2,1)$ with the complement that moves the axis. It does not rotate two compact $\mathrm{SU}(2)$ factors into each other, and in the parallel plane it preserves a chirality axis only at a trivial angle. A dual-only increment of the dual angles occupies that six-space: superposing a magnetic Faraday field, and substituting $\theta_a\mapsto\theta_a+e A_a$ in the dual rotor, are two laboratory readings of it. Their kinematics occupy Section~\ref{sec:gravitycontrol}. An extended rotor built from $\int F\,ds$ remains open. The harmonic stiffness does not furnish a Higgs reading of rotor rigidity; that fact belongs to the particle action of Section~\ref{sec:dualrotordynamics}. Neither the dual map nor the dual rotor supplies a V--A coupling.
...
Because $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ depends only on the relative rotor, dual-only motion---usual rapidities held fixed---obeys
\[
\dot J=-\sum_a\beta_a\dot\beta_a.
\]
Increasing the dual angles cannot raise $J$. At vanishing dual angle the first variation vanishes, and $J$ drops at second order along a dual ray $\beta=tv$. The admissible cone $\alpha_a,\beta_a\ge 0$, $\alpha_a+\beta_a\le\pi/2$ is convex, so affine interpolation of dual angles between admissible endpoints of equal usual rapidity remains admissible.

It is worth fixing the target state before asking how to reach it. Call a configuration \emph{gravitationally neutral}, or equivalently \emph{shielded}, when the invariant mismatch vanishes:
\begin{equation}
\label{eq:neutral-def}
J=0,\qquad\text{equivalently}\qquad\sum_a\alpha_a^2=\sum_a\beta_a^2 .
\end{equation}
The two sectors then hold equal rapidity budgets, and the particle neither attracts nor repels; $J>0$ is the dual budget falling short, $J<0$ its overshooting. Neutrality in this sense is a statement about the signed scalar and about nothing else, and it is worth separating from three notions it is easily confused with. It is not the vacuum: apart from the trivial origin a gravitationally neutral configuration carries strictly positive unsigned mass $M=\sum_a\alpha_a^2$, and the heavier the usual sector, the larger the dual budget neutrality consumes. It is not coincidence of the two rotors: $J$ can vanish while the relative rotor $\Omega$ stays away from the identity, so a neutral particle remains internally torsioned. And it is not the absence of structure: where the two rotors share a common scale $J$ vanishes while their interference continues to alternate, so the layered phase of Section~\ref{sec:strongfield} survives neutrality untouched. A gravitationally neutral particle is thus not an inert one. It is one whose gravitational and inertial signature, in the sense of Section~\ref{sec:unified}, has been cancelled, while everything else about it remains.

That cone has a dual wall $\beta_a=\pi/2-\alpha_a$, on which
\[
J=\frac\pi2\sum_a\bigl(\alpha_a-\pi/4\bigr).
\]
Dual-only $J$ cannot fall below the wall. Shielding $J=0$ by dual excitation alone is possible if and only if the wall is nonpositive, equivalently $\sum_a\alpha_a\le 3\pi/4$. Repulsion $J<0$ requires a strictly negative wall.

That budget is global. $J$ is a scalar sum, so dual rapidity parked on a silent axis cancels usual rapidity on a boosted one. The isotropic target $\beta=\alpha$ lies in the cone precisely when $\alpha_a\le\pi/4$ on every axis; dual-only shielding does not require that point. A usual rapidity $\alpha$ on one axis and the same dual rapidity on another is admissible for $0\le\alpha\le\pi/2$ and has $J=0$, so even a seed at the cone's edge $\alpha=\pi/2$ can be dual-only shielded. What dual-only cannot shield is a uniformly large boost: three axes at $\pi/3$ exhaust $\sum\alpha\le 3\pi/4$. Uniform seeds make the distinction sharp. At $\alpha=\pi/6$ the equal-scale locus is admissible, massive, and repulsive at the wall. At uniform $\alpha=\pi/3$ no dual-only configuration reaches $J=0$; a single axis at the same value still does.

The same six numbers define the unsigned mass $M=\tfrac12\sum_a(\alpha_a^2+\beta_a^2)$. Independently of any trajectory,
\[
J+M=\sum_a\alpha_a^2,\qquad M-J=\sum_a\beta_a^2.
\]
Dual-only motion travels on $J+M=\mathrm{const}$: every drop in $J$ is paid for, one-for-one, by unsigned mass, and $J=0$ forces the dual budget $\sum\beta^2=\sum\alpha^2$. When the wall vanishes that shield is unique---the wall itself. When the wall is strictly negative the same mass is shared among a family of dual angles; two silent axes already give two distinct shields. Dual-only $J$ fills the closed interval from the wall up to the pure-usual value. From a pure usual seed one lands at $M=2M_{\mathrm{seed}}$.

Usual-only motion conserves $M-J$. When the wall is positive, dual excitation cannot cancel $J$, but unwinding the usual rotor lowers the ceiling $\sum\alpha^2$ itself. Truncating each $\alpha_a$ to at most $\pi/4$ and matching the dual angle always yields a massive equal-scale shield; at uniform $\alpha=\pi/3$ that mixed unwind succeeds where dual-only fails. Dual-only conserves $\sum\alpha^2$, so the vacuum $J=M=0$ lies on no dual-only orbit except the origin. Mixed control reaches it from every seed by unwinding the usual rotor. Inertial resistance, in the sense of Section~\ref{sec:unified}, tracks $J$, not $M$. Synchronising the dual rotor with the usual one can null $J$ while increasing $M$; reducing both requires walking toward the origin.

The same dual-only ray is not a single return. On a boosted axis the factor $\gamma_{\mathrm{eff}}$ of Section~\ref{sec:timedilation} falls as the dual angle rises, and it meets the unboosted value $1$ at a unique dual angle $\beta_\ast$ whose sine is $\tanh(\alpha/2)$. That meeting precedes the coaxial shield: for every positive usual rapidity at most $\pi/2$ one has $\beta_\ast<\alpha$, so the admissible equal-scale point, where it exists, already has $\gamma_{\mathrm{eff}}<1$. Whether the wall itself can still reach $1$ is a different comparison. The equation $\cos\alpha=\tanh(\alpha/2)$ has a unique root $\alpha_\dagger$ in $(\pi/3,\pi/2)$; below that root a coaxial dual ray meets $\gamma_{\mathrm{eff}}=1$ before the wall, and above it every admissible dual angle on the boosted axis remains on the side $\gamma_{\mathrm{eff}}>1$. Cross-axis compensation, which shields the same seed at the same unsigned mass, leaves the boosted axis unexcited in the dual sector, so that axis keeps the special-relativistic factor $\cosh\alpha$ while the compensating axis contributes the trivial factor $1$. Nulling $J$ and restoring the clock are therefore two different points of the same cone.

All of that concerns trajectories. The cone itself, before any trajectory is chosen, already limits what a shield can weigh. The variables that make the limit visible are the axis sum and difference $s_a=\alpha_a+\beta_a$, $d_a=\alpha_a-\beta_a$, in which admissibility reads simply $0\le s_a\le\pi/2$ and $|d_a|\le s_a$, while the two observables become
\[
J=\tfrac12\sum_a s_ad_a,\qquad M=\tfrac14\sum_a\bigl(s_a^2+d_a^2\bigr).
\]
Mass is a squared radius; mismatch is a product of coordinates. On a single axis the cone then forces $(k^2-s_a^2)(k^2-d_a^2)\ge 0$ with $k=\pi/2$, that is
\[
2k^2\bigl(\alpha_a^2+\beta_a^2\bigr)\le k^4+\bigl(\alpha_a^2-\beta_a^2\bigr)^2 ,
\]
so an axis can be heavy only by being lopsided. Nothing forbids three axes from being lopsided in opposite senses, and that is precisely the freedom shielding exploits; but the cancellation is itself bounded, because each axis density $\alpha_a^2-\beta_a^2$ lies in $[-k^2,k^2]$. Three such numbers cannot drive the sum of their pairwise products below $-k^4$, and assembling the two facts already binds the observables in the inequality
\begin{equation}
\label{eq:shield-ceiling}
\pi^2M\le\frac{5\pi^4}{16}+4J^2,
\qquad\text{equivalently}\qquad
M_{\mathrm{norm}}\le\frac56+\frac32\,J_{\mathrm{norm}}^2 .
\end{equation}
The bound holds on the whole cone. It is sharp throughout the central band $|J|\le\pi^2/8$, which is the only band in which a shield can sit, and it is attained along the wall family
\[
\alpha=(\pi/2,\,0,\,\gamma),\qquad\beta=(0,\,\pi/2,\,\pi/2-\gamma),\qquad 0\le\gamma\le\pi/2,
\]
for which $J=\tfrac\pi2(\gamma-\pi/4)$ and $M=\tfrac{5\pi^2}{16}+(\gamma-\pi/4)^2$ turn \eqref{eq:shield-ceiling} into an identity. Read at neutrality \eqref{eq:neutral-def}, a shielded particle cannot weigh more than
\[
M\le\frac{5\pi^2}{16}=\frac56\cdot\frac{3\pi^2}{8},
\]
five sixths of the cone's own mass ceiling. The remaining sixth of the available mass is closed to neutrality---whatever trajectory is taken, and whatever force drives the dual channel. The heaviest shield in the theory is the midpoint $\gamma=\pi/4$: one axis purely usual, one axis purely dual, the third split evenly, all three on the wall.

The shape of that extremal is as telling as its weight. It is maximally anisotropic: the two sectors do not cancel axis by axis, they exchange roles across axes, and that exchange is what buys the mass. An isotropic shield $\beta=\alpha$ is confined by $\alpha_a\le\pi/4$ to $M\le 3\pi^2/16$, lighter than the optimum by exactly the factor $5/3$. Matching each dual angle to its own usual rapidity---the most natural picture of ``aligning the two rotors''---is therefore the inefficient way to shield a heavy particle. Cross-axis compensation is the efficient one.

The parabola \eqref{eq:shield-ceiling} is not, however, the envelope of the cone. The estimate on pairwise products that produces it does not use which side of zero the axis densities occupy. Once the signed sum $T=2J$ is large, that omission is costly: each density already sits near the usual wall $\alpha^2-\beta^2=k^2$, and the pairwise products cannot be as negative as $-k^4$. The same observation with the dual wall produces a repulsive companion. The three quadratic pairings together cut the cone down to
\begin{equation}
\label{eq:control-envelope}
M\le\frac{5\pi^2}{16}+\frac1{\pi^2}\min\bigl(4J^2,\,(2J-\tfrac{\pi^2}{2})^2,\,(2J+\tfrac{\pi^2}{2})^2\bigr),
\end{equation}
or equivalently
\[
M_{\mathrm{norm}}\le\frac56+\frac16\min\bigl(9J_{\mathrm{norm}}^2,\,(3J_{\mathrm{norm}}-2)^2,\,(3J_{\mathrm{norm}}+2)^2\bigr).
\]
Every pair with $|J|\le M$ beneath this envelope is realised by an admissible configuration, and no pair above it is. The kinematic domain of gravitational control is therefore this region and nothing else.

Its geometry is three-lobed. The central lobe is the shield parabola, running from $J=-\pi^2/8$ to $J=\pi^2/8$ and meeting the absolute mass ceiling $M=3\pi^2/8$ at both ends. Beyond those ends two axes lock to the same sector at the cone's edge, and the remaining wall traces a parabola centred not at $J=0$ but at $|J|=\pi^2/4$. The envelope therefore falls back to the shield value $M=5\pi^2/16$ at $|J|=\pi^2/4$, then climbs the outer lobe to the three-axis lock at $|J|=3\pi^2/8$, where the mass ceiling is met again. The outer arcs are attained by two pure-usual axes plus a wall, and by the usual--dual swap of that family. Neutrality remains the expensive crossing of the domain; there is a second, cheaper, local minimum where two axes are already locked and the third is itself a shield.

These families are the whole of the envelope. An admissible configuration meets it precisely when every axis lies on the wall and at least two axes are purely usual or purely dual. In the open central band the locked pair occupies opposite sectors, and $J$ fixes how the remaining axis is split: each assignment of the three roles determines one configuration, and the heaviest shield is the one that halves the free axis. On an open outer arc the locked pair belongs to one sector, and $J$ again fixes the rest. The wall reads the dual angles off the usual rapidities, so the boundary has no internal freedom. Dual-only motion holds those rapidities fixed and therefore meets the envelope at most once; sliding along the ceiling, from a light shield toward a two-axis lock or across neutrality, has to move the usual rotor.

At the top of the mass range the freedom vanishes altogether. A configuration attaining $M=3\pi^2/8$ must have every axis either purely usual, $(\alpha_a,\beta_a)=(\pi/2,0)$, or purely dual, $(0,\pi/2)$; at maximal radius the cone admits no intermediate. Such a configuration's mismatch is a sum of three quanta $\pm\pi^2/8$, so $|J_{\mathrm{norm}}|\ge 1/3$. The heaviest configurations the theory admits are not merely hard to shield: they are not gravitationally neutral at all, and nothing can make them so without first making them lighter. They occur at the four points where the envelope meets the mass ceiling---the two-axis locks at $|J|=\pi^2/8$ and the three-axis locks at $|J|=3\pi^2/8$---and nowhere else.

The asymmetry between the two directions deserves saying plainly, since it inverts the intuition that repulsion should be the expensive state. The ceiling in \eqref{eq:shield-ceiling} is a ceiling on neutrality, not on gravity. Full repulsion costs nothing: the pure-elliptic configuration $\alpha=0$, $\beta_a=\pi/2$ carries the maximal mass $3\pi^2/8$ together with the extreme value $J_{\mathrm{norm}}=-1$, and its attractive mirror does the same. Attraction and repulsion are available at the mass ceiling; what is obstructed is the crossing between them, and, once two axes have locked, a second crossing at $|J|=\pi^2/4$. The design constraint of a control programme is therefore not how much dual excitation can be supplied, but which lobe of the domain one is willing to occupy, and how heavy an object one is willing to leave gravitationally visible.

Brought back to the seed, the two budgets prove complementary rather than redundant. A usual seed that dual excitation alone can shield obeys the linear condition $\sum_a\alpha_a\le 3\pi/4$, and the shield it reaches carries $M=\sum_a\alpha_a^2$; the ceiling therefore imposes the quadratic condition $\sum_a\alpha_a^2\le 5\pi^2/16$ on that same seed.

An electromagnetic coupling, a resonance frequency, or a laboratory protocol is not forced by this geometry. What the geometry does force, once a laboratory reading of a dual-only increment is chosen, is the kinematics of $J$. Superposing Faraday coefficients onto the mismatch, and substituting $\theta_a\mapsto\theta_a+e A_a$ in the dual rotor, occupy the same six-space: they coincide when the increment is magnetic and $B=eA$, and they part company under a constant shift of $A$. The remainder of this section records those identities, and then a proposed helicity-odd drive that is not a consequence of either reading.

\subsection{Theoretical Coupling of Circularly Polarized Electromagnetic Fields to Dual Rotors}

The electromagnetic field occupies the six-generator space of Section~\ref{sec:unification}, partitioned as in~\eqref{eq:faraday-split}. Laboratory time reversal $(E,B)\mapsto(E,-B)$ remains a distinct involution from both duality and the usual--dual parameter swap.

A Faraday increment added to the particle's mismatch bivector is superposition in that six-space. A purely magnetic increment holds the usual rapidities fixed, so it is dual-only, conserves $J+M$, and cannot raise $J$ when the existing dual angles and the increment are nonnegative. A purely electric increment is usual-only. The circular wave of that section is mixed. It is null at every phase, and every linear Faraday coefficient has vanishing four-phase mean, so
\[
\langle F_{\mathrm{usual}}\rangle=\langle F_{\mathrm{dual}}\rangle=0.
\]
There is in particular no mean dual field proportional to $\sigma E_0^2 K$. Superposition of the wave onto a background torsion does not shift the mean of $J$. What is helicity-odd and constant is the bilinear Poynting component $(E\times B)_z=\sigma E_0^2$ of that section. Linear polarisation carries a nonnegative Poynting component and no helicity sign. The Faraday mix cannot create $J$ from the wave. A first-order rest sandwich of the dual summand vanishes for every Faraday field, circular waves included. The rest sandwich of the wave itself is the oscillating electric kick, whose four-phase mean vanishes; a velocity along the beam produces a transverse kick with no longitudinal component~\eqref{eq:faraday-lorentz}. Axis projectors $P_{L,R}$ likewise fail to extract helicity.

The dual-rotor substitution $\theta_a\mapsto\theta_a+e A_a$ is the same dual-only increment, read as a potential: it equals magnetic superposition of $eA$. Instantaneously
\begin{equation}
\label{eq:ponderomotive-J}
J=J_0-e\,\beta\cdot A-\tfrac12 e^2 A^2.
\end{equation}
For a Coulomb-gauge circular potential of helicity $\sigma^2=1$, $A^2=(E_0/\omega)^2$ is constant and independent of $\sigma$; the four-phase mean of $J$ therefore drops by the helicity-even ponderomotive $\tfrac12 e^2(E_0/\omega)^2$. A linearly polarised potential of the same $E_0$ drops the mean by half that amount. Cycle-averaged $\dot J$ vanishes: the shift is a displacement of the mean, not a secular drift. Two potentials that differ by a constant---both would have vanishing Faraday field---yield different values of $J$; the substitution is not Maxwell-gauge invariant, and that is where the two readings part company. From a vanishing dual seed a zero-mean circular $A$ makes a dual angle negative, and so leaves the admissible cone. An admissible realisation therefore requires a dual bias no smaller than the oscillation, together with remaining room to the wall. Magnetostatic dual excitation is the chamber-preserving case of the same kinematics, and the $1/\omega^2$ suppression of the ponderomotive drop is why that channel prefers slow magnetic excitation to a high-frequency wave.

Neither reading is helicity-odd in $J$. A helicity-dependent drive of the particle mismatch is therefore not a local scalar of $F$, nor a linear superposition, nor a first-order rest sandwich of $F_{\mathrm{dual}}$, nor an axis-chirality projection, nor a beam-direction Lorentz kick, nor the dual-rotor substitution $\theta\mapsto\theta+eA$. Any such drive requires extra structure---a preferred axis, a potential, or the particle's dual phase. Within the classical quadratic span the helicity-odd candidate remains $(E\times B)_z=\sigma E_0^2$. In the model below that bilinear is the source of a dual-phase drive, with right-handed polarisation ($\sigma=+1$) reinforcing attractive mismatch ($J>0$) and left-handed polarisation ($\sigma=-1$) opposing it. That identification remains a model.

If a dual-channel generalised acceleration $u$ is added to $\ddot\theta$ in the written particle action of Section~\ref{sec:dualrotordynamics}, the mismatch is a double integrator
\[
\ddot\delta=-u,
\]
with no restoring frequency. Dual-only kinematics is not a free solution of that action: holding $\ddot\phi=0$ uniquely requires the usual-channel force $v=m(\phi-\theta)$. Realising the dual-only trajectory $\theta=\theta_0+eA$ further requires $u=e\ddot A+m(\phi-\theta)$: the coupling $e\ddot A$ is not by itself the Euler--Lagrange dual source unless the mismatch already vanishes. The neighbouring same-sign oscillator instead obeys $\ddot\delta+2m\delta=-u$. Resonance remains extra structure, not an identity of the written action.

The coupling in the working model is written as a path-ordered exponential

\[
R_{\text{dual}} \to \mathcal{P} \exp\!\left( \frac{1}{2} \int (\theta_a + e A_a)\, \Gamma_a \, ds \right),
\]

where the coupling $e$ absorbs the generator scale, and the dual generators $\Gamma_a$ mediate the interaction. The phase-lock
\[
\dot{\theta} = \kappa \sigma E_0^2 \sin(\omega t - \theta + \delta_\sigma)
\]
is a model of that extra structure, not a consequence of the admissible-cone kinematics or of the written Euler--Lagrange system.

\subsection{Resonance Frequencies and the Role of Superconductors}

The intrinsic dual-spacetime scale is Planckian ($\sim 10^{43}$ Hz). Macroscopic coherence ($N \sim 10^{26}$ particles) is proposed to redshift an effective frequency to $10^{12}$--$10^{15}$ Hz. Superconductors would further reduce it via Josephson plasma resonances and collective phase locking, achieving tunable modes in the GHz--THz range accessible to current technology. That reduction is part of the working model; the written action itself supplies no Compton oscillator.

\subsection{Proposed Experimental Protocol and Predictions}

A microgram-scale test mass in a high-$Q$ superconducting resonator is illuminated with tunable circularly polarized radiation (0.1--10 THz). Expected signatures, if the dual-channel coupling exists, include helicity-dependent changes in apparent weight or inertial mass ($\Delta m/m \gtrsim 10^{-6}$), with left-handed polarization producing reduction and right-handed enhancement. A resonant peak would test the extra phase-lock structure, not the dual-only geometry of $J$, and not the helicity-even ponderomotive of~\eqref{eq:ponderomotive-J}. The algebraic channel already prefers slow magnetic dual excitation: a high-frequency circular wave is the wrong regime for that drop, and from a vanishing dual seed it is not even an admissible trajectory. Success of the chiral protocol would therefore be evidence for the extra model, not a confirmation of the dual-only increment.

If the model is right, gravity is not merely controllable but chirally controllable, with helicity serving as the switch between attraction and repulsion. The admissible cone's image in the $(J,M)$-plane is known exactly: a three-lobed domain bounded by \eqref{eq:control-envelope}. Dual-only kinematics already say which trajectories stay inside it. The dual-only increment already supplies a helicity-even, low-frequency channel along those trajectories. Whether laboratory electromagnetism supplies a helicity-odd dual force is a conjecture, to be decided in the laboratory.
...
\]
The mismatch $\delta=\phi-\theta$ is therefore free: $\ddot\delta=0$. On one axis the integral is the affine lag $\delta(t)=\delta_0+\nu t$ together with the cubic common rapidity
\[
\sigma(t)=\sigma_0+\dot\sigma_0\,t-m\delta_0\,t^2-\frac{m\nu}{3}\,t^3,
\]
and the Jacobi integral along that motion equals the initial value $\tfrac12\nu\dot\sigma_0+\tfrac m2\delta_0^2$. The written action does not yield the oscillator $\ddot\delta+m\delta=0$. A neighbouring system sometimes written alongside it,
\[
\ddot\phi=m(\phi-\theta),\qquad -\ddot\theta=m(\phi-\theta),
\]
makes the mismatch runaway, $\ddot\delta=2m\delta$. For $m>0$ that runaway is the hyperbolic motion $A\cosh(\sqrt{2m}\,t)+B\sinh(\sqrt{2m}\,t)$, unbounded for a nonzero seed, and is likewise not an oscillator.
...
\]
This is a separate working model, not the written action. Its frequency is $\sqrt{2m}$ (in natural units $\hbar=c=1$). That line and the Compton frequency $m$ meet only at the isolated values $m=0$ and $m=2$. For a free particle in uniform motion the usual rapidity is modelled as evolving at constant rate $\dot{\phi}_a=\gamma v_a/c$. The general solution is
\[
\delta(t)=A\cos(\sqrt{2m}\,t)+B\sin(\sqrt{2m}\,t),
\]
with $\delta(t)^2\le A^2+B^2$ and with Jacobi integral $\tfrac14\dot\sigma_0^2+\tfrac m2(A^2+B^2)$.
...
\]
the de Broglie phase of the matter wave. Collective averaging of this particle-intrinsic action over many particles is proposed to recover a macroscopic integral of $J$. That averaging is not an identification of $J$ with the Weitzenb\"ock density $T$. The written action does not furnish a harmonic bridge to quantum theory; the same-sign model is the only oscillator among the three systems compared here. An arbitrary potential of the mismatch does not change the opposite-sign conclusion: the lag stays free, and a constant lag is inertial only where the slope vanishes, so the harmonic stiffness accelerates any nonzero lag. A quartic well off alignment freezes a nonzero lag only when the kinetic signs agree, and the flat rate is then initial data, not a mass for $W$ or $Z$.

A dual-channel generalised acceleration $u$ added to $\ddot\theta$ does not restore a frequency in the written action: $\ddot\delta=-u$ and $\ddot\sigma=-2m\delta+u$. Those identities assume both channels free. Holding $\ddot\phi=0$ is a constraint. If a usual-channel force $v$ is admitted together with $u$, the written Jacobi integral changes at the rate $v\dot\phi-u\dot\theta$, and $\ddot\phi=0$ uniquely fixes $v=m(\phi-\theta)$. Dual-only kinematics is therefore not a free solution: leftover mismatch with nonzero rest mass drags the usual sector, and realising the dual-only slice requires that unique constraint force. When the usual rapidity is instantaneously at rest, $\dot\phi=0$, the constraint contributes no power; only the dual pairing $-u\dot\theta$ remains. The neighbouring oscillator instead obeys $\ddot\delta+2m\delta=-u$. Whether laboratory fields supply such a $u$ is the modelling question of Section~\ref{sec:gravitycontrol}, not a consequence of these identities.
...
The relative rotor is the identity precisely when the two rotors coincide. A Lorentz-invariant scalar $J$ measures the mismatch; it is bounded on the admissible cone, and vanishing $J$ does not force the rotors to agree. On every static radial-boost chart, $J$ stands in a closed relation with the Weitzenb\"ock torsion $T$ of teleparallel gravity, of which the exterior Schwarzschild geometry is the unique vacuum case. The two agree only in the flat limit. A real radial boost cannot produce repulsion; repulsive layers belong to the elliptic sector and are of bounded depth. Where the two rotors share a common scale, $J$ vanishes while their interference still alternates, so the layers of strong-field gravity are ordered by that interference and not by $J$. The same algebra houses electromagnetism: Faraday coefficients occupy the six-generator space of the dual rotor, and the sandwich increment is a Lorentz 4-force. A spatial chirality axis splits that six-space into an $\mathfrak{so}(2,1)$ stabilizer and a complement that moves the axis. A perpendicular dual rotation exchanges the two projectors at angle $\pi$, and the stabilizer's only compact direction admits no nontrivial mix with the parallel electric generator. Compact $\mathrm{SU}(2)$ is the dual rotor, not a chiral gauge symmetry. On a three-sphere chart the circular speed is $v^2=(GM/R)f(r/R)$, the shape $f$ has a unique minimum on $(0,\pi/2)$, and attraction ends at $\pi R/2$, before $r=2R$. None of this requires a background manifold.

We have proposed that the layered phase is the strong force; that galaxies are three-sphere patches of the dual sector, and that this compactness therefore accounts for flat rotation curves; that coherent excitation of the dual sector might engineer gravity; that early dual dynamics sources baryon asymmetry; that Maxwell's equations remain unrecovered from the same six-space, and that a harmonic stiffness does not select a rotor rigidity away from alignment; that a mismatch oscillator is the de Broglie phase; and that a Hilbert-space quantization of the dual rotor completes the theory. These are readings of the geometry. Dual-only motion on the admissible cone cannot raise $J$. It conserves $J+M$, so every dual-only shield from a pure usual seed doubles unsigned mass, and it reaches $J=0$ precisely when the dual wall is nonpositive. On a boosted axis a coaxial dual-only ray restores the clock factor $\gamma_{\mathrm{eff}}$ to $1$ strictly before it cancels $J$; a cross-axis shield of equal mass leaves that clock at $\cosh\alpha$. Independently of any trajectory the cone's image in the $(J,M)$-plane is the three-lobed envelope \eqref{eq:control-envelope}: the shield parabola is sharp only for $|J|\le\pi^2/8$, neutrality is capped at five sixths of the maximal mass and is reached only by a maximally anisotropic wall configuration, and at maximal mass every axis is purely usual or purely dual---while full repulsion pays no such penalty. The envelope is a lock---every axis on the wall, at least two of them purely usual or purely dual---so dual-only motion meets it at most once and cannot travel along it. Holding the usual rotor fixed against the written potential requires a constraint. A dual-only increment of the dual angles---magnetic Faraday superposition, or the substitution $\theta\mapsto\theta+eA$---cannot raise $J$ against a nonnegative seed; along a circular wave the Faraday reading leaves mean $J$ unmoved, while the potential reading drops it by a helicity-even ponderomotive that is not Maxwell-gauge invariant and that leaves the cone from a vanishing dual seed. Whether laboratory electromagnetism supplies a helicity-odd dual force---and whether gravity is therefore chirally controllable---remains a conjecture.

A balanced massive seed is not emptied by either rapidity budget alone, and on the aligned flow its mass stands still only when the common rate vanishes. A hydrogen ionisation does not pay the electron rest energy, and the first Coulombic node is not a wall.
