...
\]
The mismatch is a double integrator: no restoring frequency stands between a dual force and a change of $J$. The neighbouring same-sign oscillator instead obeys $\ddot\delta+2m\delta=-u$, so a constant drive shifts the equilibrium mismatch while accelerating the common rapidity. Resonance is therefore additional structure, not an identity of the written action.
...
If the model is right, gravity is not merely controllable but chirally controllable, with helicity serving as the switch between attraction and repulsion. Dual-only kinematics already say when shielding and repulsion are reachable on the admissible cone. Whether laboratory electromagnetism supplies the dual force is a conjecture, to be decided in the laboratory.
...
A dual-channel generalised acceleration $u$ added to $\ddot\theta$ does not restore a frequency in the written action: $\ddot\delta=-u$ and $\ddot\sigma=-2m\delta+u$. The neighbouring oscillator instead obeys $\ddot\delta+2m\delta=-u$. Whether laboratory fields supply such a $u$ is the modelling question of Section~\ref{sec:gravitycontrol}, not a consequence of these identities.
