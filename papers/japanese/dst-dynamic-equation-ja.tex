...
\end{equation}
となる。不整合は復元周波数をもたない。\(-u\) の二重積分である。それらの恒等式は両チャネルが自由であることを仮定する。\(\ddot\phi=0\) を課すと \(m(\phi-\theta)=0\) となる。残留する不整合と非零の静止質量は通常セクターを引きずるので、双対専用の運動学は書かれた作用の自由解ではない。同符号の作業模型では
\begin{equation}
