...
構成を組織するのは四つの主張である。双対性 \(X\mapsto Xi\) はローレンツセクター上の複素構造である。すべてのブーストは回転の双対であり、\(\mathfrak{so}(3,1)\) を \(\mathfrak{so}(4)\) から分ける符号は \(i^{2}=-1\) である。ねじれと並進を一つにする幾何学的対象は、単一のバナッハ指数ではなく、ねじれローターと零トランスレータの順序積 \(M=RT\) である。座標チャート上では、外部シュヴァルツシルト幾何はそのモーターの動径ブーストであり、パラメータ空間で不整合を測る有限角のスカラー \(J\) は、遠平行の場の密度 \(T\) ではない。重力の方向は並進が担う。静止観測者の運動は、ホバリング加速度の逆数だけ内側にずれた軸のまわりの、率 \(GM/r^{2}\) の単一のブーストであり、重力はその軸へ向かう。軸は地平で観測者に達し、遠方では無限遠へ退く。キリングノルムの平方のダランベルシアンは \(8J\) であるから、物体が軸へ落ちるか軸から離れるかは \(J\) の符号が決める。
\end{abstract}
...
方向は別の問題である。ラピディティの場は、コフレームがどれだけ強くブーストされているかを述べるが、放された物体がどちらへ落ちるかは述べない。答えは、チャートの読みが使わずに残した並進の側にある。静止観測者は時間並進によって動き、\(G(3,1,1)\) においてその運動の生成元は、互いに可換でない動径ブーストと時間並進の和である。その和は内側にずれた軸のまわりの単一のブーストであり、重力は観測者からその軸へ向かう。ブースト率は近似なしにニュートン場 \(GM/r^{2}\) であり、軸までの距離の逆数はホバリング加速度である。親理論が引力と斥力として読む \(J\) の符号は、キリングノルムの曲がりの符号になる。
...
\]
に沿って運ぶ。これは軸上の事象 \(-\ell e_{1}\) から一定の空間的間隔 \(\ell\) を保つ双曲線である。その動径座標は決して負にならない。軌道は軸から離れる向きに曲がる。間隔 \(\ell\) の双曲線の固有加速度は \(1/\ell\) である~\cite{Rindler1966}。キリング軌道の加速度は事象におけるキリング場とその一階微分で決まるから、これは平坦な流れのものであるだけでなく、静止観測者の真のホバリング加速度である。自由な物体は直線上を動き、観測者から見て軸へ落ちる。

事象における重力の方向は、観測者からそのキリング・バイベクトルの軸へ向かう方向である。その強さは軸までの距離の逆数である。
...
これらの主張は、事象におけるキリングジェットと、それが生成する平坦な流れに関するものである。一般の多様体に沿うキリング・バイベクトルの共変な輸送は構成しない。親理論のねじれ不整合をキリング・バイベクトルと同一視することもしない。
...
重力の方向は並進が担う。静止観測者のキリング・バイベクトルは、\(\ell=1/a\) だけ内側へ動かされた軸のまわりの、率 \(\kappa=GM/r^{2}\) の純ブーストである。ここで \(a=\partial_{r}\sqrt{A}\) はホバリング加速度であり、時間脚のヴァイツェンベックねじれである。重力はその軸へ向かう。軸は遠方では無限遠へ退き、そこで静的な運動は時間並進である。地平では軸は観測者に達し、運動は表面重力でのブーストである。赤方偏移を除けば、場の種はそのブーストのねじれスカラーである。キリングノルムの平方のダランベルシアンは \(8J\) であるから、双曲的なローレンツ部分は物体をその軸へ引き寄せ、楕円的なものは遠ざける。これが親理論の符号規則の運動学的な中身である。
