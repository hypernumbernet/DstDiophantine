...
構成を組織するのは三つの主張である。双対性 \(X\mapsto Xi\) はローレンツセクター上の複素構造である。すべてのブーストは回転の双対であり、\(\mathfrak{so}(3,1)\) を \(\mathfrak{so}(4)\) から分ける符号は \(i^{2}=-1\) である。ねじれと並進を一つにする幾何学的対象は、単一のバナッハ指数ではなく、ねじれローターと零トランスレータの順序積 \(M=RT\) である。座標チャート上では、外部シュヴァルツシルト幾何はそのモーターの動径ブーストである。パラメータ空間で不整合を測る有限角のスカラー \(J\) は、遠平行の場の密度 \(T\) ではない。
\end{abstract}
...
これら十個の要素はバイベクトル空間の基底である。したがって三つの類のスパンは零でのみ交わり、任意のバイベクトルはブーストと回転と並進の和として一意に書ける。この空間は、次元が三、三、四の直和に分かれる。数え \(\binom{5}{2}=10\) はその和の次元である。この和の交換子は次節の主題であり、分裂そのものは次数に属する。
...
ブーストと回転の6次元スパンを \(\mathfrak{l}\)、4次元零スパンを \(\mathfrak{n}\) と書く。バイベクトル空間の分裂は、この和を直和にする。二つの直和成分は零でのみ交わり、次数2の何ものもそれらの外にはない。交換子の双線型性は生成元の恒等式を
\[
...
本稿の構成は Lean~4 プロジェクト \texttt{DstDiophantine} において形式化されている。以下は第二の証明ではない。導入の三つの主張を、それを検査する定理へ結ぶ地図である。証明は指名したモジュールにあり、ここでは再現しない。\(\mathfrak{iso}(3,1)\) との抽象的な同型、一般モーターに対する場の辞書、および \(G(3,1,1)\) の完全な退化計量によるサンドイッチは、形式化では主張していない。十個の生成元の線形独立性と、バイベクトル空間の内部における三つの類の直和は、\texttt{Algebra.BivectorBasis} の \texttt{linearIndependent\_tenGen}、\texttt{poincareSpan\_eq\_bivectorGrade}、\texttt{disjoint\_lorentz\_null} である。
