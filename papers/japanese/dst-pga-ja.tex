...
構成を組織するのは四つの主張である。双対性 \(X\mapsto Xi\) はローレンツセクター上の複素構造である。すべてのブーストは回転の双対であり、\(\mathfrak{so}(3,1)\) を \(\mathfrak{so}(4)\) から分ける符号は \(i^{2}=-1\) である。ねじれと並進を一つにする幾何学的対象は、単一のバナッハ指数ではなく、ねじれローターと零トランスレータの順序積 \(M=RT\) である。座標チャート上では、外部シュヴァルツシルト幾何はそのモーターの動径ブーストであり、パラメータ空間で不整合を測る有限角のスカラー \(J\) は、遠平行の場の密度 \(T\) ではない。重力の方向は並進が担う。静止観測者の運動は、ホバリング加速度の逆数だけ内側にずれた軸のまわりの、率 \(GM/r^{2}\) の単一のブーストであり、重力はその軸へ向かう。そのバイベクトルの平坦な流れでは、静止から放たれた物体は率 \(1/\ell\) で軸へ加速し、固有時間 \(\ell\) で軸の光錐に会う。軸は地平で観測者に達し、遠方では無限遠へ退く。キリングノルムの平方のダランベルシアンは \(8J\) であるから、物体が軸へ落ちるか軸から離れるかは \(J\) の符号が決める。
\end{abstract}
...
\xi''(0) \;=\; -\frac{1}{\ell}
\]
である。初加速度はホバリング加速度の大きさをもち、軸へ向かう。同じ直線は固有時間 \(s=\ell\) で軸上の事象の光錐に会い、その事象は平坦な流れのキリング地平 \(N=0\) の上にある。軸までの距離は、放たれた物体がその地平に達する固有時間でもある。観測者の同時面に沿う隔たりが \(-\ell\) へ近づくのは \(\sigma\to\infty\) のときだけである。物体は有限の固有時間で地平に会い、ホバリングする側はその会合へ漸近する。
...
これらの主張は、事象におけるキリングジェットと、それが生成する平坦な流れに関するものであり、その流れの中で静止から放たれた物体の落下を含む。一般の多様体に沿うキリング・バイベクトルの共変な輸送は構成しない。親理論のねじれ不整合をキリング・バイベクトルと同一視することもしない。
...
重力の方向は並進が担う。静止観測者のキリング・バイベクトルは、\(\ell=1/a\) だけ内側へ動かされた軸のまわりの、率 \(\kappa=GM/r^{2}\) の純ブーストである。ここで \(a=\partial_{r}\sqrt{A}\) はホバリング加速度であり、時間脚のヴァイツェンベックねじれである。重力はその軸へ向かう。平坦な流れでは、手放した瞬間の慣性枠は、観測者から見てその軸のまわりのブーストのままである。静止から放たれた物体は直線 \(s\,e_{0}\) を進み、観測者の固有時間 \(\sigma\) における外向き法線に沿う符号つき隔たりは \(\ell(\operatorname{sech}(\sigma/\ell)-1)\) である。初加速度は \(-1/\ell\) で軸へ向かい、直線はキリングノルムが消える軸の光錐に、固有時間 \(\ell\) で会う。軸は遠方では無限遠へ退き、そこで静的な運動は時間並進であり、その固有時間は発散する。地平では軸は観測者に達し、運動は表面重力でのブーストであり、固有時間は消える。赤方偏移を除けば、場の種はそのブーストのねじれスカラーである。キリングノルムの平方のダランベルシアンは \(8J\) であるから、双曲的なローレンツ部分は物体をその軸へ引き寄せ、楕円的なものは遠ざける。これが親理論の符号規則の運動学的な中身である。

残るのは、平面に対する共変な括弧積算法、\(G(3,1,1)\) の退化計量によるサンドイッチ、一般モーターに対する場の辞書、および親理論の不整合をキリング・バイベクトルに結ぶ共変な輸送である。TEGR とアインシュタイン--ヒルベルト重力の変分等価性は~\cite{maluf2013} の入力である。
...
本稿の構成は Lean~4 プロジェクト \texttt{DstDiophantine} において形式化されている。以下は第二の証明ではない。本稿の主張を、それを検査する定理へ結ぶ地図である。証明は指名したモジュールにあり、ここでは再現しない。\(\mathfrak{iso}(3,1)\) との抽象的な同型、一般モーターに対する場の辞書、\(G(3,1,1)\) の完全な退化計量によるサンドイッチ、およびキリング・バイベクトルの共変な輸送は、形式化では主張していない。十個の生成元の線形独立性と、バイベクトル空間の内部における三つの類の直和は、\texttt{Algebra.BivectorBasis} の \texttt{linearIndependent\_tenGen}、\texttt{poincareSpan\_eq\_bivectorGrade}、\texttt{disjoint\_lorentz\_null} である。
...
\texttt{Algebra.Motor} は \(\Omega_{\mathrm{torsion}}\)、\(\Omega_{\mathrm{trans}}\)、および順序積 \(RT\) を定義する。並進生成元の冪零性はバナッハ指数を打ち切り、トランスレータは加法し、積はユニタリである：
{\small
\begin{verbatim}
...
\texttt{Gravity.KillingAxis} が第\ref{sec:direction}節を担う。トランスレータによる共役は一次で厳密である（\texttt{sandwich\_expTrans}）。静止キリング・バイベクトルはずれたブーストであり、変位は一意であり、\(K\) は二つの超平面の交わりで \(K^{2}=\kappa^{2}/4\) をみたす（\texttt{sandwich\_radialShift\_boost}、\texttt{sandwich\_expTrans\_boost\_eq\_killing\_iff}、\texttt{killingBivector\_eq\_meet}、\texttt{killingBivector\_sq}）。事象、速度、軌道は
{\small
\begin{verbatim}
theorem commutator_omegaBiv_event (p : OmegaParams) (x : Fin 4 -> Real) :
    commutator (omegaBiv p) (event x) =
      e4 * (minkowskiVector p.trans.lambda +
        commutator (omegaTorsion p.torsion) (minkowskiVector x)) * i
theorem killing_velocity_eq_zero_iff (k nu s : Real) :
    commutator (killingBivector k nu) (event ![0, s, 0, 0]) = 0
      <-> nu + k * s = 0
theorem hoverWorldline_interval (k l tau : Real) :
    Q31 (hoverWorldline k l tau - (radialShift l).lambda) = l ^ 2
\end{verbatim}
}
である。流れは軸を点ごとに固定し、軌道は軸から離れる向きに曲がる（\texttt{sandwich\_exp\_killing\_event\_axis}、\texttt{sandwich\_exp\_killing\_event\_zero}、\texttt{hoverWorldline\_radial\_nonneg}）。外部チャート上で \(\kappa=\sqrt{A}\,\partial_{r}\sqrt{A}\) および \(r^{2}\kappa=r_{s}/2\) である（\texttt{killingRate\_eq\_norm\_mul\_deriv}、\texttt{sq\_mul\_killingRate}）。\(\ell\) と \(\partial_{r}\sqrt{A}\)、\(T^{0}{}_{rt}\)、\(-\sqrt{A}\,\varphi'\) それぞれとの積はすべて一である（\texttt{axisDistance\_mul\_dSqrtA\_dr}、\texttt{axisDistance\_mul\_torsion}、\texttt{axisDistance\_mul\_rapidity}）。軸の側は質量の符号である（\texttt{axis\_neg\_iff}、\texttt{killingRate\_pos\_iff}）。単調性と二つの極限は \texttt{axisDistance\_strictMonoOn}、\texttt{tendsto\_axisDistance\_horizon}、\texttt{tendsto\_staticKilling\_horizon}、\texttt{tendsto\_axisDistance\_atTop}、\texttt{tendsto\_staticKilling\_atTop} であり、赤方偏移した場の種は \texttt{J\_killingRate} と \texttt{J\_hoverAcceleration} である。ダランベルシアン~\eqref{eq:dalembert-J} は、\texttt{commutator\_omegaTorsion\_minkowskiVector} が \([\Omega_{\mathrm{torsion}},x]\) を \(\omega x\) と同定したうえで、あらゆる事象とあらゆる刻みについて厳密な二階差分として成り立つ（\texttt{killingNormSq\_dAlembertian}）。二つの側は \texttt{killingNormSq\_killingParams\_radial} と \texttt{killingNormSq\_rotationParams} である。

平坦な流れの落下則も同じモジュールにある。ホバリングする側から見た慣性枠は逆モーター \texttt{exp\_neg\_killing\_eq\_displacedBoost} である。双曲線の接線の平方間隔は \(-(\ell\kappa)^{2}\) であり（\texttt{hoverTangent\_interval}）、固有時間はラピディティを \(\sigma/\ell\) へ再パラメータ化する（\texttt{hoverWorldline\_proper}）。同時な隔たりは~\eqref{eq:fall-displacement} の \(\xi(\sigma)\) に等しい（\texttt{released\_separation\_eq\_displacement}、\texttt{releasedDisplacement\_eq\_proper}）。\(\ell>0\) のとき、それは零でない固有時間において狭義に負である（\texttt{releasedDisplacementProper\_lt\_zero}）。手放した瞬間に値と一階導関数は消え、二階導関数は \(-1/\ell\) である（\texttt{released\_initial\_acceleration}）。外部チャート上でその加速度は \(-\partial_{r}\sqrt{A}\) である（\texttt{released\_initial\_acceleration\_exterior}）。隔たりは \(-\ell\) へ近づく（\texttt{tendsto\_releasedDisplacementProper\_atTop}）。慣性直線は固有時間 \(\ell\) で軸の光錐に会い、そこは地平 \(N=0\) である（\texttt{released\_lightcone\_iff}、\texttt{released\_on\_killingHorizon}）。
