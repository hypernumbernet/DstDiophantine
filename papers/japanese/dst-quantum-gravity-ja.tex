...
二重時空理論は根本的に代数的であるが、その量子セクターは現在または近未来の技術で検証可能な具体的実験予測を与える。不整合スカラー $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ は相対ローターのみに依存する。双対専用の運動は $J$ を上げられない。それは $J+M$ を保存するので、純通常の種からの双対専用遮蔽は符号なし質量を倍にする。遮蔽は大域的な予算 $\sum\alpha\le 3\pi/4$ であって、軸ごとの等方条件ではない。単一のブースト軸は、沈黙する軸に双対ラピディティを置くことでなお双対専用遮蔽できる。斥力は壁が真に負であることを要する。真空 $J=M=0$ は混合制御では到達可能であり、原点を除けば双対専用では到達不能である。実験室の場が双対チャネルの力を供給するかどうかは予想のままである。

\subsection{理論的結合機構}

円偏光電磁波は任意の位相でヌルであり、線形ファラデー係数の四位相平均はすべて消える。六つの古典ファラデー二次量のうち、その波の上でヘリシティ奇数な結合はポインティング成分 $(E\times B)_z=\sigma E_0^2$ に限る。粒子不整合のヘリシティ駆動は $F$ の局所スカラーでも、線形重ね合わせでも、双対和項の静止サンドイッチでもない。作業模型はその双一次量を双対位相駆動の候補源とし、右円偏光（$\sigma=+1$）は引力不整合（$J>0$）を強化し、左円偏光（$\sigma=-1$）はそれに対抗する。有効相互作用は
\[
...
\]
は余分な構造である。書かれた粒子作用は、双対チャネルの加速度 $u$ のもとで不整合を二重積分器 $\ddot\delta=-u$ とし、復元周波数を供給しない。$\ddot\phi=0$ は通常チャネルの拘束力 $v=m(\phi-\theta)$ を一意に固定する。その拘束がなければ両セクターが動く。
...
\item 有効質量のヘリシティ依存変化を探す。提案される双対ローター尺度 $\theta \sim c / (2\pi r_\text{comp})$ における共鳴ピークは、余分な位相ロック構造を試すのであって、$J$ の双対専用幾何を試すのではない。
\end{itemize}
