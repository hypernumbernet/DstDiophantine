...
本論文の残りの構成は以下のとおりである。第\ref{sec:spinor}節では平方 $+1$ の射影を構成し、合成 $P_{\mathrm{spin}}P_R$ が冪等でないことを示す。第\ref{sec:quantumstates}節では双対生成子を $-i\sigma_a$ と同一視し、すべてのラピディティにおける双対ローターを $\mathrm{SU}(2)$ 行列と同一視する。キリング対はボルン則ではない。第\ref{sec:dirac}節では $\mathrm{Cl}(3,1)$ のクリフォード関係を述べ、$\gamma^0$ と双曲生成子 $j$ を区別し、ミンコフスキー二次形式を運動量空間のディラック方程式へ因数分解し、双対スピノルから質量殻上の解を組み立てる。第\ref{sec:mass}--\ref{sec:dynamics}節では質量の値、ヒッグス機構、測定、経路積分を幾何的仮説として論じ、書かれた粒子作用が不整合を自由にし共通ラピディティを源とすることを示す。第\ref{sec:predictions}節は許容上限に縛られた予想である。第\ref{sec:comparison}節では内部双対と十次元時空超対称を書き分ける。第\ref{sec:experiments}節は近未来の実験である。第\ref{sec:status}節は代数的主張をまとめる。第\ref{sec:conclusions}節は要約である。
...
\]
であり、$n=\beta/\lVert\beta\rVert$（$\beta\neq 0$）、$U(0)=I$ である。その行列はユニタリで行列式 $1$ である。相対双対ローターは $\mathrm{SU}(2)$ に留まる。計算軸まわりの $2\pi$ 双対回転は $-I$ である。双対時空のコンパクトなトポロジー（$\phi_a \bmod 4\pi$）はこの $\mathbb{C}^2$ 上の行列群と両立する。多様体の独立な微分同相 $S^3\cong\mathrm{SU}(2)$ はここでは証明しない。
...
質量殻上で $m\neq 0$ のとき、任意の双対スピノル $u$ が解を定める。上ワイルブロックは $u$ であり、下ブロックは $p$ のカイラルスラッシュによって固定される。静止運動量 $(m,0,0,0)$ ではこれは対 $\Psi=(u,iu)$ である。同一の構成は、縦ブースト
\[
p=(m\cosh\alpha,\,0,\,0,\,m\sinh\alpha)
\]
を含む、殻上のあらゆる四元運動量に適用される。双対回転だけ異なる二つの静止系解の $\mathbb{C}^4$ 重なりは、双対ローター振幅の二倍である。その重なりは静止系の恒等式であり、時空の内積ではない。
...
次の主張は代数において成り立つ、成り立たない、または未解決である。ヒルベルト層の対は~\cite{dst-d4l} で扱う。
...
一般角の双対ローター行列 $U(\beta)\in\mathrm{SU}(2)$；相対双対ローターは $\mathrm{SU}(2)$ に留まる & 成り立つ \\
$\mathrm{Cl}(3,1)$ において $\{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}$ & 成り立つ \\
$\gamma^0$ は双曲生成子 $j$ に等しい & 偽 \\
$|\Omega|^2$ はボルン確率である & 偽~\cite{dst-d4l} \\
$\mathrm{Cl}(3,1)\cong\mathrm{Cl}(9,1)$、または双対スピノル空間は $10$ 次元マヨラナ--ワイル加群である & 偽 \\
$Q(p)$ のクリフォード因子としての運動量空間ディラック方程式 & 成り立つ \\
質量殻上のワイル構成は静止系 $(u,iu)$ を回復し、殻上のすべての $p$ を解く & 成り立つ \\
静止系ディラック重なりは双対ローター振幅の二倍 & 成り立つ \\
書かれた反対符号作用：$\ddot\delta=0$、$\ddot\sigma=-2m\delta$；ヤコビ積分は保存し不定 & 成り立つ \\
主張された EL $\ddot\phi=m(\phi-\theta)$、$-\ddot\theta=m(\phi-\theta)$ が $\ddot\delta+m\delta=0$ を与える & 偽（暴走 $\ddot\delta=2m\delta$） \\
同符号運動項：$\ddot\delta+2m\delta=0$、振動数 $\sqrt{2m}$、自由な共通ラピディティ & 作業模型として成り立つ \\
時空上のディラック偏微分方程式、質量＝ねじれ、ヒッグス機構、測定、経路積分 & 未解決 \\
左イデアルの既約性（スピノル加群として） & 未解決 \\
TEGR とアインシュタイン--ヒルベルトの変分的等価 & 文献~\cite{maluf2013} \\
\hline
\end{tabular}
\end{center}
...
二重時空理論は量子重力の代数的設定を与えるのであり、完結した導出ではない。すべての質量粒子は $16$ 次元双四元数代数 $\mathbb{H}\oplus\mathbb{H}\cong\mathrm{Cl}(3,1)$ に符号化される。第\ref{sec:status}節に集めた恒等式は、$J$ の許容上限、平方 $+1$ の射影、同軸のみの可換、表現 $-i\sigma$、すべての双対ラピディティにおけるロドリゲス行列、および $\mathrm{Cl}(3,1)$ のクリフォード関係を含む。いくつかの自然な同一視は成り立たない。キリング対はボルン則ではなく、$I$ は $i\sigma$ ではなく、$\gamma^0$ は $j$ ではなく、$P_{\mathrm{spin}}P_R$ は射影ではなく、$\mathrm{Cl}(3,1)$ は $\mathrm{Cl}(9,1)$ ではない。

時空上のディラック偏微分方程式、ねじれ剛性としてのフェルミオン質量、ヒッグス機構、測定、もつれは本論文の定理ではない。運動量空間のディラック方程式はミンコフスキー二次形式の一次因子であり、質量殻上では静止系の対 $(u,iu)$ を回復する双対スピノルによって解かれる。書かれた反対符号作用は不整合を自由にし共通ラピディティを源とし、保存するヤコビ積分は不定である。近傍の振動子が現れるのは同符号の運動項に限り、振動数は $\sqrt{2m}$ であってコンプトン振動数 $m$ ではない。自由な遅れをド・ブロイ位相と同定することは読みのままである。通常／双対ローターの非整列としての重力は伴う重力論文のプログラムのままである。TEGR とアインシュタイン--ヒルベルトの等価は文献から採用する。上限 $|J|\le 3\pi^2/8$ は許容構成上のねじれを縛るが、それだけで天体物理的特異点を禁止するわけではない。
