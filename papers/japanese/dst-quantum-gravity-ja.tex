...
調和剛性はヒッグス機構ではない。標準模型ではヒッグス場が対称点から外れた真空期待値を獲得し、そこでの曲率が質量を生む。ここで書かれたポテンシャルが最小となるのは整列である。粒子作用の反対の運動符号の下では、あらゆる定数の遅れが自由であり、遅れが慣性的であるのは傾きが消えるところに限る。したがって零でない調和的な遅れは、非整列へ落ち着くのではなく、共通ラピディティを加速する。原点から外れた四次の井戸は零でない遅れを凍らせうるが、それは二つの運動符号が一致するときに限る。そのとき共通位相は平坦であり、その速さは初期データであって、\(W\) や \(Z\) の質量ではない。ワインバーグ角は生じない。
