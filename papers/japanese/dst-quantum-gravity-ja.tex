...
本論文の残りの構成は以下のとおりである。第\ref{sec:spinor}節では平方 $+1$ の射影を構成し、合成 $P_{\mathrm{spin}}P_R$ が冪等でないことを示す。第\ref{sec:quantumstates}節では双対生成子を $-i\sigma_a$ と同一視し、軸 $0$ 双対ローターを $\mathrm{SU}(2)$ 行列と同一視する。Killing 対は Born 則ではない。第\ref{sec:dirac}節では $\mathrm{Cl}(3,1)$ のクリフォード関係を述べ、$\gamma^0$ と双曲生成子 $j$ を区別し、ミンコフスキー二次形式を運動量空間のディラック方程式へ因数分解する。第\ref{sec:mass}--\ref{sec:dynamics}節では質量の値、ヒッグス機構、測定、経路積分を幾何的仮説として論じる。第\ref{sec:predictions}節は許容上限に縛られた予想である。第\ref{sec:comparison}節では内部双対と十次元時空超対称を書き分ける。第\ref{sec:experiments}節は近未来の実験である。第\ref{sec:status}節は代数的主張をまとめる。第\ref{sec:conclusions}節は要約である。
...
四元運動量 $p$ に対し、次数1の元 $\gamma(p)=\sum_\mu p_\mu\gamma^\mu$ は、$\mathrm{Cl}(3,1)$ においても $\mathbb{C}^4$ 上の行列としても、ミンコフスキー二次形式 $Q(p)=-p_0^2+p_1^2+p_2^2+p_3^2$ に平方する。したがって質量殻 $Q(p)=-m^2$ は $(\gamma(p)-im)(\gamma(p)+im)$ の消滅である。$\gamma(p)\Psi=im\Psi$ を満たすスピノルはその殻の上にある。これが運動量空間のディラック方程式である。ワイル表示では $\gamma(p)$ はブロック反対角であり、質量が零なら二つのカイラルブロックは分離し、質量が非零ならそれらは結合する。カイラリティ $\gamma^5$ は質量 $m$ の解を質量 $-m$ の解へ送る。静止系の解は存在する。

時空上の偏微分方程式、質量項としての双対ローター遅れ、完全ローター群の下での共変性はここでは導かない。平方 $+1$ 射影 $P_L=(1-e_1)/2$、$P_R=(1+e_1)/2$ は代数内のカイラル分割を与えるが、それだけでは標準模型の弱い相互作用を与えない。
...
\]
を通じてディラック方程式の質量の値を与える、という幾何的読みをもつ。
ねじれ不整合バイベクトルは $\Omega_{\mathrm{biv}}=\log(R_{\mathrm{usual}}^\dagger R_{\mathrm{dual}})$ である。したがってフェルミオン質量は、通常ローターに対する双対ローター遅れの期待値に直接比例する。
...
$(1+e_1)/2$ と $(1+e_0 e_2)/2$ の積は冪等；左イデアルの実次元は $8$ & 成り立つ \\
...
一般角の双対ローター行列 $U(\beta)\in\mathrm{SU}(2)$；純双対バナッハ指数と一致 & 成り立つ \\
...
$Q(p)$ のクリフォード因子としての運動量空間ディラック方程式 & 成り立つ \\
時空上のディラック偏微分方程式、質量＝ねじれ、ヒッグス機構、測定、経路積分 & 未解決 \\
左イデアルの既約性（スピノル加群として） & 未解決 \\
...
時空上のディラック偏微分方程式、ねじれ剛性としてのフェルミオン質量、ヒッグス機構、測定、もつれは本論文の定理ではない。運動量空間のディラック方程式は第\ref{sec:dirac}節のミンコフスキー二次形式の一次因子である。通常／双対ローターの非整列としての重力は伴う重力論文のプログラムのままである。TEGR と Einstein--Hilbert の等価は文献から採用する。上限 $|J|\le 3\pi^2/8$ は許容構成上のねじれを縛るが、それだけで天体物理的特異点を禁止するわけではない。
