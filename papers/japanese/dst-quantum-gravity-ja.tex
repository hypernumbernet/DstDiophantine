...
二重時空理論は根本的に代数的であるが、その量子セクターは現在または近未来の技術で検証可能な具体的実験予測を与える。不整合スカラー $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ は相対ローターのみに依存する。双対専用の運動は $J$ を上げられない。遮蔽に達するのは双対壁が非正のときに限り、斥力は壁が真に負のときに限る。実験室の場が双対チャネルの力を供給するかどうかは予想のままである。
...
\]
は余分な構造である。書かれた粒子作用は、双対チャネルの加速度 $u$ のもとで不整合を二重積分器 $\ddot\delta=-u$ とし、復元周波数を供給しない。
...
要約すると、双対専用の運動学は、いつ $J$ を遮蔽または反転できるかをすでに区切る。自然が実験室の双対力を供給するか——したがって重力が制御可能なねじれ自由度であるか——が、今後十年の実験的問いである。
