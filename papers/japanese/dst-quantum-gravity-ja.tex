...
\frac{1+g}{2}
\]
は冪等である。形式展開で用いる作業用因子は、双曲射影
\[
P_{\mathrm{spin}} = \frac{1+j}{2}
\]
（$j^2=+1$）および相補的な空間カイラル対
\[
P_L = \frac{1-e_1}{2},\qquad P_R = \frac{1+e_1}{2}
\]
（$e_1^2=+1$）であり、$P_L^2=P_L$、$P_R^2=P_R$、$P_L+P_R=1$ を満たす。因子が適切に可換（または結合）するとき、合成原始冪等元を
\[
...
\]
の形をとり、$a_0,a_1,b_i,c_i \in \mathbb{R}$ である。擬スカラー $i$ を量子力学の虚数単位と同一視すると、$\psi$ は Weyl基底における標準的な四成分複素列スピノルとなる。最初の二つの成分は通常時空（左巻き）セクターに対応し、残りの成分は双対時空（右巻き）セクターを符号化する。双対写像 $X \mapsto X i$ はなお時間配向を反転しカイラリティを幾何的に符号化するが、代数的射影子そのものは上の平方 $+1$ 因子 $P_L$、$P_R$ であり、$(1\pm i)/2$ ではない。
...
双対写像 $X \mapsto X i$ は時間配向を反転する。第~\ref{sec:spinor}節の平方 $+1$ 射影子 $P_L=(1-e_1)/2$、$P_R=(1+e_1)/2$ と合わせて、スピノル空間をカイラルセクターへ組織する。したがって左巻き成分は通常セクターのブーストに優先的に結合し、右巻き成分は双対セクターの回転に結合する。このカイラル非対称性は追加の仮定ではなく、射影子を平方 $+1$ の生成子から構成したうえで、双対時空の時間反転幾何から直接従う。
