...
連続体仮説を完全に棄却することにより、本理論は古典一般相対性理論の特異点を自動的に解消する。双対時空のコンパクト性はローター角を離散化する。許容構成上では代数的上限は $|J|\le 3\pi^2/8$（同値に $|J_{\rm norm}|\le 1$）であり、構成が許容のままである限り、$r=0$ またはビッグバンにおいて現れるであろう非有界曲率を禁止する。事象地平も同様に存在しない。光線は共鳴トンネルを介して有限のねじれ層を横断する。一般相対性理論からの強場におけるずれは原理上観測可能となる。許容錐上の双対専用運動学は、いつ $J$ を遮蔽または反転できるかを区切り、双対予算が沈黙する軸へ迂回しうることを述べる。実験室の場がその双対力を供給するかどうかは、第\ref{sec:experiments}節の実験的問いである。

本論文の残りの構成は以下のとおりである。第\ref{sec:spinor}節では平方 $+1$ の射影を構成し、合成 $P_{\mathrm{spin}}P_R$ が冪等でないことを示す。第\ref{sec:quantumstates}節では双対生成子を $-i\sigma_a$ と同一視し、すべてのラピディティにおける双対ローターを $\mathrm{SU}(2)$ 行列と同一視する。キリング対はボルン則ではない。第\ref{sec:dirac}節では $\mathrm{Cl}(3,1)$ のクリフォード関係を述べ、$\gamma^0$ と双曲生成子 $j$ を区別し、ミンコフスキー二次形式を運動量空間のディラック方程式へ因数分解し、双対スピノルから質量殻上の解を組み立て、静止系の対の縦ブーストをその構成と同一視する。第\ref{sec:mass}--\ref{sec:dynamics}節では調和剛性が非整列を選ばないことを記し、測定と経路積分を幾何的仮説として論じ、書かれた粒子作用が不整合を自由にし共通ラピディティを源とすることを示す。第\ref{sec:predictions}節は許容上限に縛られた予想である。第\ref{sec:comparison}節では内部双対と十次元時空超対称を書き分ける。第\ref{sec:experiments}節は近未来の実験である。第\ref{sec:status}節は量子セクターの代数的描像をまとめる。第\ref{sec:conclusions}節は要約である。
...
\]
は冪等でない。$e_0 e_1$ と $e_1$ は反可換であり、因子は可換でなく $(P_{\mathrm{spin}}P_R)^2\neq P_{\mathrm{spin}}P_R$ である。可換な平方 $+1$ 対は $e_1$ と軸外ブースト $e_0 e_2$ であり、その積射影は冪等で、生成する左イデアルの実次元は $8$ である。そのイデアルは四次元のミンコフスキー核 $\mathrm{Cl}(3,1)\,P$ と零生成子 $e_4$ によるコピーとの直和であり、後者は $G(3,1,1)$ の真部分加群なので、八次元イデアルは PGA 加群として可約である。ディラック・スピノル $\mathbb{C}^4$ の実次元との一致はこの零方向の倍加であって、既約ディラック加群との同一視ではない。ミンコフスキー核の $\mathrm{Cl}(3,1)$ 既約性は主張しない。
...
二つのセクターが整列すれば、反対符号の二次形式は打ち消し合う。その打ち消しは観測されるフェルミオン質量を生成せず、\(J\) の許容上限は電弱スケールではない。不整合の調和剛性は、整列から外れた真空を選ばない。
...
調和剛性はその機構ではない。標準模型ではヒッグス場が対称点から外れた真空期待値を獲得し、そこでの曲率が質量を生む。ここで書かれたポテンシャルが最小となるのは整列である。粒子作用の反対の運動符号の下では、あらゆる定数の遅れが自由であり、遅れが慣性的であるのは傾きが消えるところに限る。したがって零でない調和的な遅れは、非整列へ落ち着くのではなく、共通ラピディティを加速する。原点から外れた四次の井戸は零でない遅れを凍らせうるが、それは二つの運動符号が一致するときに限る。そのとき共通位相は平坦であり、その速さは初期データであって、\(W\) や \(Z\) の質量ではない。ワインバーグ角は生じない。コンパクトな双対セクターは、許容錐上の運動学的な不整合をなお有界にする。その上限はプランクスケールから電弱スケールを導くことではなく、欠けた真空の代用でもない。
...
\]
である。両角はともに加速する。したがって不整合は自由である：$\ddot\delta_a=0$。ポテンシャルが源とするのは共通ラピディティであり、$\ddot\sigma_a=-2m\delta_a$ である。自由な不整合は直ちに積分される。一軸の一般解はアフィン遅れ $\delta(t)=\delta_0+\nu t$ であり、共通ラピディティは三次式
\[
\sigma(t)=\sigma_0+\dot\sigma_0\,t-m\delta_0\,t^2-\frac{m\nu}{3}\,t^3
\]
である。凍った遅れは $\sigma$ を一様に加速し、零でない速さは加速そのものを時間について一次にする。この運動に沿ってヤコビ積分は初期値 $\tfrac12\nu\dot\sigma_0+\tfrac m2\delta_0^2$ へ潰れる。$\dot\sigma$ の二次の成長とポテンシャルの二次の成長が相殺するので、ラピディティが逃げても電荷は有限に留まる。$\delta$ の剛性として書かれた質量は $\sigma$ への力であり、振動数を復元しない~\cite{dst-dynamic-equation}。
...
振動子が現れるのは、両運動項の符号を正にとったときに限る。密度 $\tfrac12\dot\phi^2+\tfrac12\dot\theta^2-\tfrac m2(\phi-\theta)^2$ は $\ddot\delta+2m\delta=0$ を与え、単位 $\hbar=c=1$ で振動数 $\sqrt{2m}$ であり、共通ラピディティは自由である。一般解は $\delta(t)=A\cos(\sqrt{2m}\,t)+B\sin(\sqrt{2m}\,t)$ であり、$\delta(t)^2\le A^2+B^2$、ヤコビ積分は $\tfrac14\dot\sigma_0^2+\tfrac m2(A^2+B^2)$ に等しい。その振動数とコンプトン振動数 $m$ が一致するのは孤立した値 $m=0$ と $m=2$ に限り、模型は書かれた作用でもない。$m\ge 0$ ではそのヤコビ積分は非負であり、保存は $\delta$ の運動を縛る。同じ速さの近傍の符号パターン $\ddot\delta=2m\delta$ は、代わりに非有界な双曲線運動 $A\cosh(\sqrt{2m}\,t)+B\sinh(\sqrt{2m}\,t)$ である。
...
二重時空理論はテレパラレル重力との等価性を通じてアインシュタイン方程式を正確に回復し、$G(3,1,1)$ の可換射影子は八次元左イデアルを生成するが、それは零生成子で分解され既約フェルミオンとしては読まない。離散性は集合論的ではなく代数的（ローター角上のディオファントス制約）であり、因果性は光的な $J=0$ 共鳴条件によって保存される。熱力学的類推は呼び出されない。時空の創発は粒子内在的双対ローター間のねじれ整列の集団効果である。
...
二重時空理論は根本的に代数的であるが、その量子セクターは現在または近未来の技術で検証可能な具体的実験予測を与える。不整合スカラー $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ は相対ローターのみに依存する。双対専用の運動は $J$ を上げられない。それは $J+M$ を保存するので、純通常の種からの双対専用遮蔽は符号なし質量を倍にする。遮蔽は大域的な予算 $\sum\alpha\le 3\pi/4$ であって、軸ごとの等方条件ではない。単一のブースト軸は、沈黙する軸に双対ラピディティを置くことでなお双対専用遮蔽できる。斥力は壁が真に負であることを要する。真空 $J=M=0$ は混合制御では到達可能であり、原点を除けば双対専用では到達不能である。双対角の双対専用増分は $J$ においてヘリシティ偶数である。円偏光ファラデー波は平均を動かさず、円偏光ポテンシャルはポンデロモーティブにそれを下げ、消える双対種からは錐を出る。実験室の場がヘリシティ奇数の双対チャネルの力を供給するかどうかは予想のままである。

\subsection{理論的結合機構}

円偏光電磁波は任意の位相でヌルであり、線形ファラデー係数の四位相平均はすべて消える。その波を不整合へ重ねても平均 $J$ は動かない。六つの古典ファラデー二次量のうち、その波の上でヘリシティ奇数な結合はポインティング成分 $(E\times B)_z=\sigma E_0^2$ に限る。同一の双対専用増分を $\theta\mapsto\theta+eA$ として読めば、ヘリシティ偶数のポンデロモーティブ $\tfrac12 e^2\langle A^2\rangle$ だけ平均 $J$ を下げ、マックスウェルゲージ不変ではない。粒子不整合のヘリシティ駆動は、その増分でも $F$ の局所スカラーでもない。作業模型はポインティング双一次量を双対位相駆動の候補源とし、右円偏光（$\sigma=+1$）は引力不整合（$J>0$）を強化し、左円偏光（$\sigma=-1$）はそれに対抗する。有効相互作用は
\[
H_\text{int} = \kappa \sigma E_0^2 \sum_a \theta_a \Gamma_a
\]
と書かれ、$E_0$ は電場振幅、$\kappa$ は物質依存の結合であり、いずれも双対ローター代数から導かれない。位相ロック
\[
\dot{\theta} = \kappa \sigma E_0^2 \sin(\omega t - \theta + \delta_\sigma)
\]
は余分な構造である。書かれた粒子作用は、双対チャネルの加速度 $u$ のもとで不整合を二重積分器 $\ddot\delta=-u$ とし、復元周波数を供給しない。双対専用の運動学はその作用の拘束スライスである。その拘束がなければ両セクターが動く。
...
\item 有効質量のヘリシティ依存変化を探す。提案される双対ローター尺度 $\theta \sim c / (2\pi r_\text{comp})$ における共鳴ピークは、余分な位相ロック構造を試すのであって、$J$ の双対専用幾何を試すのでもなく、双対専用増分のヘリシティ偶数ポンデロモーティブを試すのでもない。
\end{itemize}

結合が存在するならば、予測される効果の大きさは、達成可能な強度 $10^{12}$--$10^{15}$ W/m$^2$ において $\Delta m / m \sim 10^{-2}$--$10^{-5}$ である。兆候は特徴的である。効果は偏光ヘリシティを反転すると符号を反転しなければならず、直線偏光または無偏光の放射に対しては消滅しなければならない。

\subsection{反証可能性と注意点}

系統的な周波数およびヘリシティ掃引の後、$10^{-6}$ 水準での帰無結果は双対ローター結合強度を制約するのであって、$J$ の許容錐運動学を制約するのではない。積極的検出——とりわけヘリシティ可逆の質量シフト——は、重力と慣性が双対時空に起源するねじれ現象であることの強い証拠をなすであろう。共鳴ピークはさらに作業振動子または位相ロックを制約する。書かれた作用はそれを強制しない。

本理論は即時の技術的突破を主張しない。上記の提案は控えめで卓上規模の実験であり、その主目的は双対時空自由度が実験室電磁場に測定可能に結合するという仮説を検証することである。この水準での成功はより大規模な重力工学への扉を開くであろう。失敗は単に結合模型のパラメータを制限するだけである。

\subsection{より広い含意}

双対ローター制御が実行可能であることが示されれば、同一の機構は原子核内部のねじれ層遷移を駆動することによる核変換および放射能中和への経路を提供する。しかしこれらの応用は現在の量子重力検証プログラムの範囲を超え、今後の研究で扱われる。

要約すると、双対専用の運動学は、いつ $J$ を遮蔽または反転できるかを、いかなる符号なし質量のコストにおいてかを、双対予算が沈黙する軸へ迂回しうることを、すでに区切る。自然が実験室の双対力を供給するか——したがって重力が制御可能なねじれ自由度であるか——が、今後十年の実験的問いである。
...
符号 $(-,+,+,+)$ のディラック行列は同一代数の次数1生成子である。それらは平方 $+1$ の射影を組む双曲バイベクトルではない。$\gamma^0$ の平方は $-1$ であり、$P_{\mathrm{spin}}$ の生成子 $j$ の平方は $+1$ である。フェルミオンの運動学はしたがって代数の内部にある。可換な平方 $+1$ の射影子は $G(3,1,1)$ に八次元左イデアルを生成し、それは四次元のミンコフスキー核と零コピーに分かれ、後者は真部分加群なので PGA 加群として可約である。実次元が $\mathbb{C}^4$ と一致するのはその倍加によるのであり、既約ディラック加群との同一視ではない。双対スピノルは巡回生成子に対する $\mathbb{C}^2$ ベクトルであり、ディラックの担体は質量殻上でそれから組み立てられる。四元運動量のクリフォード平方はミンコフスキー二次形式 $Q(p)$ である。運動量空間のディラック方程式はその二次形式の一次因子である。質量殻上では双対スピノル $u$ が解を定め、静止運動量では対 $\Psi=(u,iu)$ である。$u$ の双対回転はその殻に留まる。静止系の対の縦ブーストは、ブースト軸に沿うカイラル因子の後の同一のワイル構成であり、その因子はブースト軸まわりの双対回転と可換であり、直交軸まわりの双対回転とは必ずしも可換でない。双対回転だけ異なる二つの静止系解の重なりは、双対ローター振幅の二倍である。これは時空上の偏微分方程式ではなく、完全ローター群の下での共変性ではなく、$m$ の値をねじれから導くことでもない。
...
書かれた反対符号の粒子作用は、この運動学から量子理論への動力学的な橋を閉じない。そのオイラー--ラグランジュ系は不整合を自由にし、共通ラピディティを源とする。$\ddot\delta=0$、$\ddot\sigma=-2m\delta$ である。積分はアフィン遅れと三次の共通ラピディティであり、その運動に沿うヤコビ積分は初期データに還元される。積分は不定であり、保存は振動数を回復しない。振動子が現れるのは両運動項の符号を正にとったときに限り、そのとき振動数は $\sqrt{2m}$ であり、コンプトン振動数 $m$ と一致するのは孤立した値 $m=0$ と $m=2$ に限る。自由な遅れの速さをド・ブロイ位相と同定することは読みのままである。調和剛性は非整列を選ばない。零でない遅れが慣性的であるのは、ポテンシャルの傾きが消えるところに限る。同期としての測定、もつれ、経路積分は、この運動学の上に置かれた幾何的仮説である。代数の帰結ではない。
