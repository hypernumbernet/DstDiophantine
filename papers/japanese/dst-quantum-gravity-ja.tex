...
二重時空理論は根本的に代数的であるが、その量子セクターは現在または近未来の技術で検証可能な具体的実験予測を与える。不整合スカラー $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ は相対ローターのみに依存する。双対専用の運動は $J$ を上げられず、許容錐上で遮蔽または斥力に達するのは双対壁が非正のときに限る。実験室の場が双対チャネルの力を供給するかどうかは予想のままである。
