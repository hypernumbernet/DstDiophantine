...
連続体仮説を完全に棄却することにより、本理論は古典一般相対性理論の特異点を自動的に解消する。双対時空のコンパクト性はローター角を離散化する。許容構成上では代数的上限は $|J|\le 3\pi^2/8$（同値に $|J_{\rm norm}|\le 1$）であり、構成が許容のままである限り、$r=0$ またはビッグバンにおいて現れるであろう非有界曲率を禁止する。事象地平も同様に存在しない。光線は共鳴トンネルを介して有限のねじれ層を横断する。一般相対性理論からの強場におけるずれは原理上観測可能となる。許容錐上の双対専用運動学は、いつ $J$ を遮蔽または反転できるかを区切る。実験室の場がその双対力を供給するかどうかは、第\ref{sec:experiments}節の実験的問いである。
...
二重時空理論は根本的に代数的であるが、その量子セクターは現在または近未来の技術で検証可能な具体的実験予測を与える。不整合スカラー $J=\tfrac12\sum_a(\alpha_a^2-\beta_a^2)$ は相対ローターのみに依存する。双対専用の運動は $J$ を上げられない。それは $J+M$ を保存するので、純通常の種からの双対専用遮蔽は符号なし質量を倍にする。遮蔽に達するのは双対壁が非正のときに限り、斥力は壁が真に負のときに限る。実験室の場が双対チャネルの力を供給するかどうかは予想のままである。
...
\]
は余分な構造である。書かれた粒子作用は、双対チャネルの加速度 $u$ のもとで不整合を二重積分器 $\ddot\delta=-u$ とし、復元周波数を供給しない。$\ddot\phi=0$ を課すと $m(\phi-\theta)=0$ となる。通常ローターへの拘束がなければ、両セクターが動く。
...
要約すると、双対専用の運動学は、いつ $J$ を遮蔽または反転できるかを、いかなる符号なし質量のコストにおいてかを、すでに区切る。自然が実験室の双対力を供給するか——したがって重力が制御可能なねじれ自由度であるか——が、今後十年の実験的問いである。
