...
二つのセクターが整列すれば、反対符号の二次形式は打ち消し合う。その打ち消しは観測されるフェルミオン質量を生成せず、\(J\) の許容上限は電弱スケールではない。
...
調和剛性はヒッグス機構ではない。標準模型ではヒッグス場が対称点から外れた真空期待値を獲得し、そこでの曲率が質量を生む。ここで書かれたポテンシャルが最小となるのは整列である。粒子作用の反対の運動符号の下では、あらゆる定数の遅れが自由であり、遅れが慣性的であるのは傾きが消えるところに限る。したがって零でない調和的な遅れは、非整列へ落ち着くのではなく、共通ラピディティを加速する。原点から外れた四次の井戸は零でない遅れを凍らせうるが、それは二つの運動符号が一致するときに限る。そのとき共通位相は平坦であり、その速さは初期データであって、\(W\) や \(Z\) の質量ではない。ワインバーグ角は生じない。一軸上でその整列は \(J=\frac12\delta\sigma=0\)、\(M=\sigma^2/4\) であり、真空であるのは原点においてのみである。静止した凍った井戸は質量 \(v^2/4\) の釣り合い質量状態である。
