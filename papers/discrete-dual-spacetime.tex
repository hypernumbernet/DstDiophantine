...
\begin{itemize}
  ...
  \item A ceiling band of spectral holes. On one axis the second maximum in the triangle is $(K-1)^2$, separated from the wall value $K^2$ because a positive pair on the hypotenuse $n+m=K$ satisfies $|n-m|\le K-2$. The three-axis shoulder $S(K)=3K^2-2K+1$ is attained, and every integer strictly between $S(K)$ and $3K^2$ is empty. For $K\ge 2$ that positive band contains $2K-2$ slots; at $N=8$ it is exactly $|\Delta|\in\{10,11\}$. Interior holes, such as $|\Delta|=20$ at $K=3$, lie below the shoulder. The normalized width of the band tends to zero along multiples of $4$, so density in $[-1,1]$ survives.
...
\end{equation}
The bound interval is not fully occupied. The integers immediately beneath the ceiling are empty for a uniform reason, recorded once that ceiling is identified.
...
The same-wall criterion leaves open how close a configuration may approach the ceiling without attaining it. The one-axis mismatch factors as $n^2-m^2=(n-m)(n+m)$. On the triangle $n+m\le K$ the product is at most $(n+m)^2\le K^2$, with equality only at a pure wall. If the pair is not a wall, either $n+m\le K-1$, in which case the product is at most $(K-1)^2$, or the pair lies on the hypotenuse $n+m=K$ with both coordinates positive. There $|n-m|\le K-2$, so the product is at most $K(K-2)=K^2-2K$, which sits strictly below $(K-1)^2=K^2-2K+1$. The open interval between the second maximum $(K-1)^2$, attained at the shortened wall $(K-1,0)$, and the wall value $K^2$ is therefore empty on a single axis.

A three-axis sum can exceed the shoulder
\[
S(K)=2K^2+(K-1)^2
\]
only by placing every axis on the same pure wall. For $K\ge 1$ this shoulder is the integer $S(K)=3K^2-2K+1$. The pure-wall configuration jumps from $S(K)$ to the ceiling $3K^2$ and skips every intermediate integer. The shoulder itself is attained, by two axes at $(K,0)$ and one axis at $(K-1,0)$, and likewise by the sign-reversed elliptic walls.

\begin{theorem}[Ceiling band]
\label{thm:ceiling-band}
Let $K=\lfloor N/4\rfloor$. Every integer $\Delta$ with $S(K)<|\Delta|<3K^2$ is unoccupied by admissible configurations. For $K\ge 2$ there are exactly $2K-2$ such positive integers. For $K=1$ the open interval $(S(1),3)$ contains no integer, in agreement with the full occupation of $\{-3,\ldots,3\}$.
\end{theorem}

Thus the holes $|\Delta|\in\{10,11\}$ at $N=8$ ($K=2$, $S(2)=9$) are the entire ceiling band. At $N=12$ ($K=3$, $S(3)=22$) the same band accounts for $|\Delta|\in\{23,24,25,26\}$; the further vacancy $|\Delta|=20$ lies strictly below the shoulder and is an interior hole, not forced by the isolation of the wall. In the normalized scalar the distance from the shoulder to the ceiling is $(2K-1)\cdot 16/(3N^2)$. Along multiples of $4$, where $K=N/4$, this distance equals $(8N-16)/(3N^2)$ and tends to zero, so the band does not obstruct density of the occupied spectrum in $[-1,1]$. At each finite $N$ the discrete spectrum nevertheless retains holes: the ceiling band for every $K\ge 2$, together with interior holes such as $|\Delta|=20$ at $K=3$.
...
\begin{itemize}
  ...
  \item \textbf{Discrete spectra}: Gravitational wave echoes and nuclear energy levels acquire discrete torsional contributions on the occupied slots of the lattice~\eqref{eq:spectrum}. Not every multiple of $\varepsilon_N$ inside the bound is realised. Immediately below the ceiling, Theorem~\ref{thm:ceiling-band} leaves a band of $2K-2$ empty slots on each side, of normalized width $(2K-1)\cdot 16/(3N^2)$. That width vanishes along multiples of $4$, while the missing slots themselves remain part of the finite-$N$ spectrum. Interior holes below the shoulder are independent of this band.
...
The discretization of dual spacetime completes the rejection of the continuum, yielding a bounded torsion scalar on the admissible cone, a sharp discrete ceiling governed by $\lfloor N/4\rfloor$ and attained only at same-wall configurations, an explicit triangular count of admissible points, a universal attained floor $\varepsilon_N=16/(3N^2)$, a three-fold sumset spectrum whose ceiling is isolated from the attained shoulder $S(K)=3K^2-2K+1$ by a band of $2K-2$ empty positive slots whenever $K\ge 2$, a finite discrete rotor image that is not a product of one-axis images, and a Diophantine search space. Cyclic rotor exponentials are $2\pi$-periodic; hyperbolic ones are not, so torus quantization of boosts remains a modelling choice. Future work includes constructing an actual integer order (whose unit group need not be finite) and explicit descent arguments connecting macroscopic GR solutions to Planck-scale integer configurations. Admissibility, the rotor-image finiteness, and any nuclear-chart cutoff remain logically independent of one another.
