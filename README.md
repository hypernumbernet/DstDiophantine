...
| `BealEven.lean` | フェーズ 7k–7r：偶二一致の和型／差型分割；差因数核；和／差セクター干渉 |
| `BealGaussianCube.lean` | フェーズ 7k–7o：equal-odd 立方切片・Affine 組立・Mordell `y²=x³-1728` 包装・`|u|=1` 一般化 |
| `BealFinite.lean` | 有限箱（底 ≤8・指数 3…5）＋冪判定（底 ≤20・指数 3…6／≤19・指数 3…7・`C` 非有界）＋ファインダ；既知非互素解の回帰 |
...
- **P0（次サイクルの既定）**  
  ...
  Beal 有限箱の一段拡大（底 21 または指数 8）、開残件フィルタの拡大。
- **還元の現状**  
  組立は sorry なし。閉じた切片は Darmon–Merel 3位置、偶和型 `z=3`、`e=3` の `|·|=1`／mod-8 など。
  Affine←Mordell 包装と偶差完全冪抽出は 7o で組立済み（本体の階数・Fermat 下降は未）。
  有限探索は古典冪判定 ≤20（および ≤19·7）、正立方核 ≤100、開残件 ≤50。
  D4L L2/L3 とビール残件地図で未閉鎖 U を固定（開残件 U ⇏ 釣り合い座席）。
  開いているのは細残件本体。無条件古典 Beal は主張しません。
- **P1 以降**  
  論文 `papers/` 同期、差型形の整理。本体・modular bridge・Gravity
