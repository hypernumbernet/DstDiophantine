...
**検証の現状。** 振幅、四状態、結合子の非爆発、二つの順序、Killing 重なり、異軸干渉は証明済みです。構文・指定値・帰結と、二値が担えない五つの例（否定固定点、非爆発、`J_norm < 1`、相補性、ディオファントス・レジーム）も機械検証しました。離散振幅・増幅力学・巻数の二測定、および第二測定 `mass`（真空と釣り合い質量。第五ラベルではない）まで含みます。双対ローター行列は任意の双対ラピディティで \(\mathrm{SU}(2)\) に属します。草稿の射影子 \((1\pm i)/2\) と \(P_{\mathrm{spin}}P_R\) は冪等でなく、可換な平方 \(+1\) 対の積は冪等で、その左イデアルの実次元は少なくとも \(2\) です。\(\mathrm{Cl}(3,1)\) のクリフォード関係はディラック・スピノル \(\mathbb{C}^4\) 上の行列として実現します（方程式は導出しません）。入口は `DstDiophantine/Logic.lean` です（`Basic.lean` には入れません）。代表論文は `papers/dst-4-valued-logic.tex`（日本語 `papers/japanese/dst-4-valued-logic-ja.tex`）です。
